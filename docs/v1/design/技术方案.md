...
| **OAuth认证** | 微信公众号 OAuth 2.0 | - | 用户身份识别（详见 [FastAuth接入方案](../security/FastAuth接入方案.md) §3.1-3.3） |
...
### 5.1.0.1 主路径用户旅程（OAuth + 绑定 - 推荐，85%用户）

```mermaid
journey
    title 会员用户旅程 - OAuth + 星球绑定（主路径 85%）
    section 发现阶段
      看到训练营海报: 5: 会员
      产生参与兴趣: 4: 会员
      点击H5报名链接: 5: 会员
...
### 5.1.0.2 降级路径用户旅程（固定二维码 - 备用，12%用户）

```mermaid
journey
    title 会员用户旅程 - 固定二维码支付（降级路径 12%）
    section 发现阶段
      看到训练营海报: 5: 会员
      产生参与兴趣: 4: 会员
      扫描固定收款码: 4: 会员
...
| `bind_method` | VARCHAR(20) | user_fill/manual |
...
> **详细设计**：参见 [FastAuth接入方案](../security/FastAuth接入方案.md) §3（整体架构）和 §2.8（API 接口文档）

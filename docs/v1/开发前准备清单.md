...
**接口清单**（参考 FastAuth接入方案 §2.8）:
1. `GET /api/member/verify?planetUserId=xxx&nickname=xxx` - 会员验证
2. `GET /api/member/search?keyword=xxx` - 搜索会员
3. `POST /api/member/sync/manual` - 手动触发同步
4. `GET /api/member/sync/status` - 查询同步状态
5. `POST /api/member/verify/batch` - 批量验证会员

...
| 类型 | 指标 | 目标值 | 验证方法 |
|------|------|--------|----------|
| 功能 | 批量退款成功率 | ≥ 95% | 测试用例 + 生产验证 |
| 功能 | 防重复退款 | 100% 拦截 | 单元测试 |
| 性能 | 单笔退款响应时间 | < 3s | 集成测试 |
| 可用性 | 退款记录持久化 | 100% | 写入后立即读取验证 |
| 安全 | 证书文件权限 | 600 | 部署检查 |
...
| 场景 | 方案 | 说明 |
|------|------|------|
| 单进程 | 内存队列 | v0.1 为单进程，暂无并发问题 |
| 多进程（未来） | 文件锁 (flock) | 写入时加排他锁 |
| 分布式（未来） | Redis 分布式锁 | 迁移到 v1 后使用 |
...
| 级别 | 使用场景 | 示例 |
|------|----------|------|
| ERROR | 退款失败、系统异常 | 微信 API 返回错误、证书失效 |
| WARN | 重试成功、降级处理 | 第 2 次重试成功 |
| INFO | 关键业务节点 | 退款发起、退款成功、记录保存 |
| DEBUG | 调试信息 | 请求参数、响应详情 |
...
| 字段 | 脱敏规则 | 示例 |
|------|----------|------|
| transaction_id | 保留前4后4 | 4200****7890 |
| wechat_refund_id | 保留后8位 | ****01234567 |
| planet_nickname | 保留首字符 | 球** |

### 关键埋点

| 埋点事件 | 触发时机 | 上报字段 |
|----------|----------|----------|
| refund_batch_start | 批量退款开始 | camp_id, total_count |
| refund_single_start | 单笔退款开始 | out_refund_no, amount |
| refund_single_result | 单笔退款结果 | out_refund_no, status, duration_ms |
| refund_batch_complete | 批量退款完成 | success_count, failed_count, total_duration |

### 告警配置

| 告警名称 | 触发条件 | 级别 | 通知渠道 |
|----------|----------|------|----------|
| 退款成功率异常 | 单批次成功率 < 90% | P1 | 企微机器人 |
| 证书即将过期 | 证书有效期 < 30天 | P2 | 邮件 |
| 备份失败 | 备份写入失败 | P2 | 企微机器人 |
| JSON 文件损坏 | 读取失败 | P0 | 电话 + 企微 |
...
| 外部依赖 | Mock 方式 | 说明 |
|----------|-----------|------|
| 微信支付 API | nock / jest.mock | 模拟各种响应 |
| fs-extra | jest.mock | 模拟文件读写 |
| uuid | jest.mock | 返回固定值 |

### 单元测试

| 模块 | 测试文件 | 覆盖场景 |
|------|----------|----------|
| wework-pay.service.js | `wework-pay.service.unit.test.js` | 正常退款、金额错误、订单不存在、超时 |
| refund-record.service.js | `refund-record.service.unit.test.js` | 增删改查、防重复、备份 |
...
| 测试类型 | 触发时机 | 阻断条件 |
|----------|----------|----------|
| 单元测试 | 每次 push | 测试失败 或 覆盖率 < 80% |
| 集成测试 | PR 合并前 | 测试失败 |
| Lint 检查 | 每次 push | lint 报错 |
...
- [ ] **Task 6**: 更新 Swagger/API 文档 (0.5h)
  - 新增退款相关接口文档
  - 更新 `/api-docs` 路由

- [ ] **Task 7**: 更新 .env.example (0.5h)
  - 添加微信支付配置示例
  - 添加退款相关配置

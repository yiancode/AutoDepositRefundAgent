# 状态枚举定义

> **文档目的**：统一定义系统中所有状态枚举值，确保前后端、数据库、文档的一致性
> **维护原则**：本文档为**单一数据源(SSOT)**，其他文档引用此处定义

---

## 🔤 命名约定

> **重要**：本系统在不同层级使用不同的命名风格，开发时请注意区分。

### 命名规则对照表

| 层级 | 命名风格 | 示例 | 说明 |
|------|----------|------|------|
| **Java 枚举常量** | SCREAMING_SNAKE_CASE | `CampStatus.PENDING` | Java 语言规范，枚举常量全大写 |
| **Java 枚举值 (value)** | snake_case | `"pending"` | 存储在数据库和API传输的实际值 |
| **数据库字段值** | snake_case | `'pending'` | 与 Java 枚举值保持一致 |
| **API 请求/响应** | snake_case | `"bindStatus": "pending"` | JSON 字段值使用小写 |
| **TypeScript 枚举** | SCREAMING_SNAKE_CASE | `BindStatus.PENDING` | 与 Java 保持一致的命名风格 |

### 代码使用示例

```java
// ✅ 正确：Java 代码中使用大写枚举常量
if (payment.getBindStatus() == BindStatus.PENDING) {
    // 业务逻辑
}

// ✅ 正确：数据库查询时枚举自动转换为小写值
// MyBatis-Plus @EnumValue 注解会自动处理
// SQL: SELECT * FROM payment_record WHERE bind_status = 'pending'

// ✅ 正确：API 响应中返回小写值
// Jackson @JsonValue 注解自动序列化
// JSON: {"bindStatus": "pending"}
```

```typescript
// ✅ 正确：TypeScript 中使用大写枚举
if (order.bindStatus === BindStatus.PENDING) {
    // 业务逻辑
}

// ✅ 正确：API 请求体中使用小写字符串值
const request = {
    bindStatus: 'pending'  // 或 BindStatus.PENDING (值为 'pending')
};
```

### 枚举转换机制

```
┌─────────────────┐    @EnumValue     ┌─────────────────┐
│  Java 枚举常量   │ ───────────────→ │   数据库字段值    │
│ BindStatus.     │    自动转换       │   'pending'     │
│   PENDING       │ ←─────────────── │                 │
└─────────────────┘                   └─────────────────┘
        │
        │ @JsonValue
        ↓
┌─────────────────┐
│   API JSON值    │
│  "pending"      │
└─────────────────┘
```

> **注意**：在编写用户故事、技术文档时，代码示例应使用 `CampStatus.PENDING` 形式；在描述 API 响应或数据库值时，应使用 `"pending"` 形式。

---

## 📋 核心状态枚举

### 1. bind_status - 支付绑定状态

**数据库字段**: `payment_record.bind_status`
**Java枚举**: `com.camp.enums.BindStatus`
**前端枚举**: `src/enums/bindStatus.ts`

| 枚举值 | 含义 | 触发条件 | 后续操作 |
|--------|------|---------|---------|
| `pending` | 待绑定 | 支付成功，7天内待用户主动绑定 | 用户绑定 或 7天后转expired |
| `completed` | 已完成 | ① 用户主动绑定<br>② 人工审核通过 | 无（终态） |
| `expired` | 已过期 | 支付后7天未绑定 | 转入人工审核 |
| `manual_review` | 待人工审核 | 绑定超时或需人工确认 | 管理员审核 |
| `closed` | 已关闭 | 人工审核判定无法匹配 | 无（终态） |

#### 状态机图

```mermaid
stateDiagram-v2
    [*] --> pending: 支付成功
    pending --> completed: 用户主动绑定
    pending --> expired: 超过7天未绑定
    expired --> manual_review: 转入人工审核
    manual_review --> completed: 人工审核通过
    manual_review --> closed: 人工审核拒绝
    completed --> [*]
    closed --> [*]

    note right of pending
        ⏰ bind_deadline = 支付成功 + 7天
        🔑 accessToken状态 = active
        📝 用户可主动绑定星球账号
    end note

    note right of completed
        ✅ 已关联星球用户
        🔑 accessToken状态 = bound
        📝 可查看群二维码
    end note

    note right of expired
        ⚠️ 超时未绑定
        👤 转入人工审核流程
        📝 管理员处理
    end note

    note right of manual_review
        👤 需要管理员判断
        📝 待管理后台审核
    end note
```

#### 状态转换规则

| 从状态 | 到状态 | 条件 | 触发者 | bind_method |
|--------|--------|------|--------|-------------|
| pending | completed | 用户填写星球信息 | 用户H5操作 | `user_fill` |
| pending | expired | NOW() > bind_deadline | 定时任务(每日02:00) | 保持NULL |
| expired | manual_review | 绑定超时 | 定时任务 | 保持NULL |
| manual_review | completed | 管理员审核通过 | 管理员 | `manual` |
| manual_review | closed | 管理员审核拒绝 | 管理员 | 保持NULL |

#### Java枚举定义

```java
// src/main/java/com/camp/enums/BindStatus.java
package com.camp.enums;

import com.baomidou.mybatisplus.annotation.EnumValue;
import com.fasterxml.jackson.annotation.JsonValue;
import lombok.Getter;

@Getter
public enum BindStatus {
    PENDING("pending", "待绑定"),
    COMPLETED("completed", "已完成"),
    EXPIRED("expired", "已过期"),
    MANUAL_REVIEW("manual_review", "待人工审核"),
    CLOSED("closed", "已关闭");

    @EnumValue
    @JsonValue
    private final String value;
    private final String description;

    BindStatus(String value, String description) {
        this.value = value;
        this.description = description;
    }

    public static BindStatus fromValue(String value) {
        for (BindStatus status : values()) {
            if (status.value.equals(value)) {
                return status;
            }
        }
        throw new IllegalArgumentException("Unknown bind_status: " + value);
    }
}
```

#### TypeScript枚举定义

```typescript
// frontend/h5-member/src/enums/bindStatus.ts
export enum BindStatus {
  PENDING = 'pending',
  COMPLETED = 'completed',
  EXPIRED = 'expired',
  MANUAL_REVIEW = 'manual_review',
  CLOSED = 'closed'
}

export const BindStatusLabels: Record<BindStatus, string> = {
  [BindStatus.PENDING]: '待绑定',
  [BindStatus.COMPLETED]: '已完成',
  [BindStatus.EXPIRED]: '已过期',
  [BindStatus.MANUAL_REVIEW]: '待人工审核',
  [BindStatus.CLOSED]: '已关闭'
}

export const BindStatusColors: Record<BindStatus, string> = {
  [BindStatus.PENDING]: 'warning',
  [BindStatus.COMPLETED]: 'success',
  [BindStatus.EXPIRED]: 'danger',
  [BindStatus.MANUAL_REVIEW]: 'primary',
  [BindStatus.CLOSED]: 'info'
}
```

---

### 2. pay_status - 支付状态

**数据库字段**: `payment_record.pay_status`
**Java枚举**: `com.camp.enums.PayStatus`

| 枚举值 | 含义 | 说明 |
|--------|------|------|
| `pending` | 待支付 | 订单已创建，等待支付 |
| `success` | 支付成功 | 微信支付回调确认 |
| `failed` | 支付失败 | 微信支付失败或订单超时 |
| `refunded` | 已退款 | 退款成功后更新 |

**注意**：`pay_status` 与 `bind_status` 统一使用小写风格，与数据库默认值一致。

#### Java枚举定义

```java
package com.camp.enums;

import com.baomidou.mybatisplus.annotation.EnumValue;
import lombok.Getter;

@Getter
public enum PayStatus {
    PENDING("pending", "待支付"),
    SUCCESS("success", "支付成功"),
    FAILED("failed", "支付失败"),
    REFUNDED("refunded", "已退款");

    @EnumValue
    private final String value;
    private final String description;

    PayStatus(String value, String description) {
        this.value = value;
        this.description = description;
    }
}
```

---

### 3. bind_method - 绑定方式

**数据库字段**: `payment_record.bind_method`
**Java枚举**: `com.camp.enums.BindMethod`

| 枚举值 | 含义 | 触发场景 | 置信度 |
|--------|------|---------|--------|
| `h5_bindplanet` | H5绑定 | H5 主路径：OAuth 登录后直接写入绑定关系 | 100% |
| `user_fill` | 用户填写 | 降级路径：支付成功后在 H5 页面补填星球信息 | 100% |
| `manual` | 人工匹配 | 超时未绑定或信息异常时由管理员手动关联 | 100% |

**NULL值含义**：未完成绑定（bind_status=pending/expired/manual_review时）

#### Java枚举定义

```java
package com.camp.enums;

import com.baomidou.mybatisplus.annotation.EnumValue;
import lombok.Getter;

@Getter
public enum BindMethod {
    H5_BINDPLANET("h5_bindplanet", "H5绑定"),
    USER_FILL("user_fill", "用户填写"),
    MANUAL("manual", "人工匹配");

    @EnumValue
    private final String value;
    private final String description;

    BindMethod(String value, String description) {
        this.value = value;
        this.description = description;
    }
}
```

---

### 4. refund_status - 退款状态

**数据库字段**: `refund_record.refund_status`
**Java枚举**: `com.camp.enums.RefundStatus`

| 枚举值 | 含义 | 说明 |
|--------|------|------|
| `pending` | 待处理 | 退款申请已创建，待审核 |
| `approved` | 审核通过 | 管理员审核通过，待执行退款 |
| `rejected` | 审核拒绝 | 管理员拒绝退款申请 |
| `processing` | 处理中 | 正在调用微信退款API |
| `success` | 退款成功 | 微信退款完成 |
| `failed` | 退款失败 | 微信退款失败，待重试 |

#### 状态机图

```mermaid
stateDiagram-v2
    [*] --> pending: 创建退款申请
    pending --> approved: 管理员审核通过
    pending --> rejected: 管理员审核拒绝
    approved --> processing: 调用微信退款API
    processing --> success: 退款成功
    processing --> failed: 退款失败
    failed --> processing: 重试(最多3次)
    success --> [*]
    rejected --> [*]
    failed --> [*]: 超过重试次数
```

---

### 5. camp_status - 训练营状态

**数据库字段**: `training_camp.status`
**Java枚举**: `com.camp.enums.CampStatus`

| 枚举值 | 含义 | 说明 |
|--------|------|------|
| `draft` | 草稿 | 训练营创建中，未发布 |
| `pending` | 待发布 | 编辑完成，等待发布 |
| `enrolling` | 报名中 | 已发布，接受报名 |
| `ongoing` | 进行中 | 训练营已开始 |
| `ended` | 已结束 | 训练营结束，等待结算 |
| `settling` | 结算中 | 正在处理退款 |
| `archived` | 已归档 | 退款完成，已归档 |

#### 状态机图

```mermaid
stateDiagram-v2
    [*] --> draft: 创建训练营
    draft --> pending: 编辑完成
    pending --> enrolling: 管理员发布
    enrolling --> ongoing: 到达开始日期(定时任务)
    ongoing --> ended: 到达结束日期(定时任务)
    ended --> settling: 管理员触发结算
    settling --> archived: 退款完成
    archived --> [*]

    draft --> draft: 编辑中
    pending --> draft: 撤回修改
    enrolling --> enrolling: 报名中
```

---

### 6. accessToken状态

**存储位置**: Redis (`access_token:{token}`)
**数据结构**: JSON
```json
{
  "orderNo": "ord_xxx",
  "campId": 1,
  "wechatUserId": 10001,
  "status": "active",
  "expireAt": "2025-12-31T23:59:59"
}
```

| 状态值 | 含义 | 说明 |
|--------|------|------|
| `inactive` | 未激活 | 订单已创建，未支付 |
| `active` | 已激活 | 支付成功，可用于绑定操作 |
| `bound` | 已绑定 | 绑定完成，仅可查询状态 |
| `expired` | 已过期 | 超过有效期或训练营结束+7天 |

#### 状态机图

```mermaid
stateDiagram-v2
    [*] --> inactive: 创建订单
    inactive --> active: 支付成功
    active --> bound: 绑定完成
    active --> expired: 超过有效期
    bound --> expired: 超过有效期
    expired --> [*]

    note right of inactive
        未支付，token不可用
    end note

    note right of active
        已支付，可进行绑定操作
        有效期：训练营结束 + 7天
    end note

    note right of bound
        已绑定，仅可查询
        不可重复绑定
    end note
```

---

### 7. checkin_status - 打卡合格状态

**数据库字段**: `camp_member.checkin_status`
**Java枚举**: `com.camp.enums.CheckinStatus`

| 枚举值 | 含义 | 说明 |
|--------|------|------|
| `pending` | 待统计 | 训练营进行中，打卡统计未完成 |
| `qualified` | 合格 | 打卡天数达到退款条件 |
| `unqualified` | 不合格 | 打卡天数未达到退款条件 |

**合格判定公式**：`实际打卡天数 >= (要求天数 - 宽限天数)`

**关联字段**：
- `camp_member.checkin_count`: 实际打卡天数
- `camp_member.full_attendance`: 是否全勤 (boolean)
- `training_camp.required_days`: 要求打卡天数
- `training_camp.grace_days`: 宽限天数

#### Java枚举定义

```java
package com.camp.enums;

import com.baomidou.mybatisplus.annotation.EnumValue;
import lombok.Getter;

@Getter
public enum CheckinStatus {
    PENDING("pending", "待统计"),
    QUALIFIED("qualified", "合格"),
    UNQUALIFIED("unqualified", "不合格");

    @EnumValue
    private final String value;
    private final String description;

    CheckinStatus(String value, String description) {
        this.value = value;
        this.description = description;
    }
}
```

---

### 8. sync_status - 同步任务状态

**数据库字段**: `sync_log.status`
**Java枚举**: `com.camp.enums.SyncStatus`

| 枚举值 | 含义 | 说明 |
|--------|------|------|
| `running` | 进行中 | 同步任务正在执行 |
| `success` | 成功 | 同步完成，无错误 |
| `partial` | 部分成功 | 部分数据同步成功，部分失败 |
| `failed` | 失败 | 同步失败（重试耗尽或严重错误） |
| `cancelled` | 已取消 | 手动取消或系统中断 |

#### Java枚举定义

```java
package com.camp.enums;

import com.baomidou.mybatisplus.annotation.EnumValue;
import lombok.Getter;

@Getter
public enum SyncStatus {
    RUNNING("running", "进行中"),
    SUCCESS("success", "成功"),
    PARTIAL("partial", "部分成功"),
    FAILED("failed", "失败"),
    CANCELLED("cancelled", "已取消");

    @EnumValue
    private final String value;
    private final String description;

    SyncStatus(String value, String description) {
        this.value = value;
        this.description = description;
    }
}
```

---

### 9. match_status - 匹配状态

**数据库字段**: `camp_member.match_status`
**Java枚举**: `com.camp.enums.MatchStatus`

| 枚举值 | 含义 | 说明 |
|--------|------|------|
| `pending` | 待匹配 | 会员信息已创建，待匹配星球用户 |
| `matched` | 已匹配 | 已成功匹配到星球用户 |
| `failed` | 匹配失败 | 无法匹配到星球用户，需人工处理 |

**触发场景**：
- `pending`: 会员创建时的初始状态
- `matched`: 成功关联到 `planet_user` 表的用户
- `failed`: 星球信息错误或不存在时

#### Java枚举定义

```java
package com.camp.enums;

import com.baomidou.mybatisplus.annotation.EnumValue;
import lombok.Getter;

@Getter
public enum MatchStatus {
    PENDING("pending", "待匹配"),
    MATCHED("matched", "已匹配"),
    FAILED("failed", "匹配失败");

    @EnumValue
    private final String value;
    private final String description;

    MatchStatus(String value, String description) {
        this.value = value;
        this.description = description;
    }
}
```

---

### 10. audit_status - 审核状态

**数据库字段**: `refund_record.audit_status`
**Java枚举**: `com.camp.enums.AuditStatus`

| 枚举值 | 含义 | 说明 |
|--------|------|------|
| `pending` | 待审核 | 退款申请已创建，等待管理员审核 |
| `approved` | 审核通过 | 管理员审核通过，允许退款 |
| `rejected` | 审核拒绝 | 管理员拒绝退款申请 |

**与 refund_status 的关系**：
- `audit_status` 关注审核层面（管理员决策）
- `refund_status` 关注整个退款流程（包括审核、执行、结果）
- `audit_status=approved` 时，`refund_status` 才能从 `pending` 转为 `processing`

#### Java枚举定义

```java
package com.camp.enums;

import com.baomidou.mybatisplus.annotation.EnumValue;
import lombok.Getter;

@Getter
public enum AuditStatus {
    PENDING("pending", "待审核"),
    APPROVED("approved", "审核通过"),
    REJECTED("rejected", "审核拒绝");

    @EnumValue
    private final String value;
    private final String description;

    AuditStatus(String value, String description) {
        this.value = value;
        this.description = description;
    }
}
```

---

## 🔒 数据一致性约束

### 1. bind_status 与 bind_method 的关联规则

| bind_status | bind_method 允许值 | 说明 |
|-------------|------------------|------|
| `pending` | NULL | 未绑定，不存在绑定方式 |
| `completed` | `h5_bindplanet` / `user_fill` / `manual` | **必填**，记录绑定来源 |
| `expired` | NULL | 超时未绑定 |
| `manual_review` | NULL | 待审核，未确定绑定方式 |
| `closed` | NULL | 已关闭，无绑定方式 |

### 2. pay_status 与 bind_status 的关联规则

| pay_status | bind_status 允许值 | 说明 |
|------------|------------------|------|
| `pending` | 不存在 | 未支付时不产生bind_status |
| `success` | `pending` / `completed` / `expired` / `manual_review` / `closed` | 支付成功后才有绑定状态 |
| `failed` | 不存在 | 支付失败，无绑定流程 |
| `refunded` | 任意 | 退款不影响已有绑定状态 |

### 3. 数据库约束建议

```sql
-- payment_record表约束
ALTER TABLE payment_record
ADD CONSTRAINT chk_bind_status CHECK (
    bind_status IN ('pending', 'completed', 'expired', 'manual_review', 'closed')
);

ALTER TABLE payment_record
ADD CONSTRAINT chk_bind_method CHECK (
    bind_method IN ('h5_bindplanet', 'user_fill', 'manual') OR bind_method IS NULL
);

ALTER TABLE payment_record
ADD CONSTRAINT chk_pay_status CHECK (
    pay_status IN ('pending', 'success', 'failed', 'refunded')
);

-- 业务逻辑约束（应用层保证）
-- 规则：bind_status=completed 时 bind_method 必须非NULL
-- 规则：bind_status!=completed 时 bind_method 必须为NULL
```

---

## 📝 接口响应示例

### 示例1：查询订单状态（待绑定）

```json
GET /api/h5/orders/ord_123/status

Response:
{
  "code": 200,
  "data": {
    "orderNo": "ord_123",
    "payStatus": "success",
    "bindStatus": "pending",
    "bindMethod": null,
    "remainingBindTime": 518400,  // 6天(秒)
    "bindDeadline": "2025-12-10T23:59:59",
    "campName": "21天早起训练营",
    "amount": 99.00,
    "accessToken": "tk_xxx",
    "bindingUrl": "/bind?token=tk_xxx"
  }
}
```

### 示例2：查询订单状态（已完成）

```json
GET /api/h5/orders/ord_456/status

Response:
{
  "code": 200,
  "data": {
    "orderNo": "ord_456",
    "payStatus": "success",
    "bindStatus": "completed",
    "bindMethod": "user_fill",
    "boundAt": "2025-12-04T15:30:00",
    "planetUserId": "123456789",
    "planetNickname": "小明",
    "groupQrcodeUrl": "https://cdn.example.com/qrcode.jpg",
    "campName": "21天早起训练营",
    "amount": 99.00
  }
}
```

### 示例3：管理后台会员列表

```json
GET /api/admin/camps/1/members

Response:
{
  "code": 200,
  "data": {
    "items": [
      {
        "memberId": 10001,
        "wechatNickname": "小明同学",
        "planetNickname": "小明",
        "bindStatus": "completed",
        "bindMethod": "user_fill",
        "payAmount": 99.00,
        "checkinCount": 18,
        "eligibleForRefund": false
      },
      {
        "memberId": 10002,
        "wechatNickname": "小红",
        "planetNickname": null,
        "bindStatus": "manual_review",
        "bindMethod": null,
        "payAmount": 99.00,
        "checkinCount": 20,
        "eligibleForRefund": true
      }
    ],
    "total": 2,
    "page": 1,
    "pageSize": 20
  }
}
```

---

## ⚠️ 常见错误

### 错误1：使用非标准状态值

```diff
- "bindStatus": "success"  // ❌ 错误：success不是标准值
+ "bindStatus": "completed"  // ✅ 正确：使用completed
```

### 错误2：bind_status=completed但bind_method为NULL

```diff
{
  "bindStatus": "completed",
- "bindMethod": null  // ❌ 错误：完成状态必须有绑定方式
+ "bindMethod": "user_fill"  // ✅ 正确
}
```

### 错误3：bind_status=pending但bind_method有值

```diff
{
  "bindStatus": "pending",
- "bindMethod": "user_fill"  // ❌ 错误：未完成不应有绑定方式
+ "bindMethod": null  // ✅ 正确
}
```

### 错误4：混淆pay_status和bind_status

```diff
// payment_record表
{
- "pay_status": "completed",  // ❌ 错误：pay_status应为 success
+ "pay_status": "success",    // ✅ 正确
  "bind_status": "completed"  // ✅ 正确
}
```

---

## 🔄 状态变更日志表

所有状态变更必须记录到对应的状态日志表：

| 业务状态 | 日志表 | 记录字段 |
|---------|--------|---------|
| bind_status | `payment_bind_status_log` | from_status, to_status, bind_method, operator_id, reason |
| camp_status | `camp_status_log` | from_status, to_status, event, operator_id |
| refund_status | `refund_status_log` | from_status, to_status, operator_id, remark |
| pay_status | `payment_status_log` | from_status, to_status, transaction_id |

**日志记录示例**：

```java
// 记录bind_status变更
PaymentBindStatusLog log = PaymentBindStatusLog.builder()
    .paymentRecordId(paymentRecord.getId())
    .fromStatus(BindStatus.PENDING)
    .toStatus(BindStatus.COMPLETED)
    .bindMethod(BindMethod.USER_FILL)
    .operatorId(userId)
    .reason("用户主动绑定")
    .build();
bindStatusLogMapper.insert(log);
```

---

## 📚 相关文档

- [技术方案 - 5.4.1 状态机设计](./技术方案.md#541-状态机设计)
- [数据库设计 - payment_record表](./数据库设计.md#42-payment_record-支付记录表)
- [接口文档 - 1.5 状态枚举](./接口文档.md#15-状态枚举)

---

**文档版本**：v1.3
**最后更新**：2025-12-14
**维护者**：技术架构组
**变更历史**：
- 2025-12-14: 新增"命名约定"章节，明确 Java/API/数据库的大小写规范
- 2025-12-08: 新增 match_status、audit_status 枚举定义
- 2025-12-05: 新增 checkin_status、sync_status 枚举定义
- 2025-12-04: 初始版本，统一定义所有状态枚举

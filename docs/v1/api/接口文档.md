...
> **SSOT引用**: [状态枚举定义.md](../design/状态枚举定义.md) - 接口中涉及的所有状态枚举值（bind_status、bind_method、pay_status、refund_status、camp_status 等）
...
**票据交付方式**：
- 前端在支付发起后轮询 `GET /orders/{orderNo}/status`（无需票据）
...
| GET | /orders/{orderNo}/status | 查询订单状态 | 否* |
| GET | /orders/{orderNo}/params | 获取支付参数 | 否 |
| POST | /orders/{orderNo}/bind-planet | 用户绑定星球信息 | **票据** |
| GET | /orders/{orderNo}/qrcode | 获取群二维码 | **票据** |
...
> - *`/orders/{orderNo}/status` 无需票据，支付成功时响应中包含 `accessToken`，作为后续接口的票据来源
...
| POST | /members/sync | 导入知识星球会员(Excel) | JWT |
| GET | /members/sync/status | 查询导入状态 | JWT |
| GET | /members/sync/history | 查询导入历史 | JWT |
...
| GET | /checkins/sync/status/{taskId} | 查询批量同步进度 | JWT |
...
| POST | /users | 创建用户 | JWT |
| PUT | /config | 更新配置 | JWT |
| GET | /notifications | 获取通知记录列表 | JWT |
| GET | /notifications/stats | 获取通知统计 | JWT |
| POST | /notifications/{id}/resend | 手动重发通知 | JWT |
...
```bash
GET /api/h5/orders/ord_a1b2c3d4-5678-90ab-cdef/params
```
...
>
> **可选增强**：如需更高安全性，可启用一次性 Ticket 机制，详见 [支付安全增强方案](../security/支付安全增强方案.md) 阶段2。

**请求**

```
GET /api/h5/orders/{orderNo}/status
GET /api/h5/orders/{orderNo}/status?ticket={ticket}  // 可选：启用ticket验证
```
...
```bash
GET /api/h5/orders/ord_a1b2c3d4-5678-90ab-cdef/status
```
...
| payStatus | string | 是 | 支付状态：pending/success/failed |
| bindStatus | string | 否 | 绑定状态：pending/completed/expired/manual_review（支付成功后返回） |
| bindMethod | string | 否 | 绑定方式：h5_bindplanet/user_fill/manual（绑定完成后返回） |
| payTime | string | 否 | 支付时间（支付成功后返回） |
| memberId | number | 否 | 会员ID（绑定完成后返回） |
| showQrcode | boolean | 否 | 是否显示群二维码（绑定完成后为true） |
| bindDeadline | string | 否 | 绑定截止时间（bindStatus=pending时返回） |
| bindingUrl | string | 否 | 绑定页面URL，包含token参数（bindStatus=pending时返回） |
| accessToken | string | 否 | 访问令牌，用于后续接口认证（支付成功后返回） |

> **说明**：
> - `accessToken` 仅在支付成功后返回，待支付状态时为 `null`
> - `bindingUrl` 是前端绑定页面的完整URL，已包含token参数，前端可直接跳转
...
```json
{
  ...
  "message": "请先完成星球信息绑定",
  "data": {
    "bindUrl": "/api/h5/orders/{orderNo}/bind-planet"
  },
  "timestamp": 1730000000
}
```
...
|------|------|------|------|
| campId | long | 否 | 训练营ID |
| bindStatus | string | 否 | 绑定状态：pending/completed/expired/manual_review/closed |
...
```bash
GET /api/admin/members?campId=1&bindStatus=pending&page=1&pageSize=20
```
...
> **数据来源说明**：星球会员数据通过Excel离线导入（从知识星球后台导出），避免API Token过期问题。
...
| 参数 | 类型 | 必填 | 说明 |
|------|------|------|------|
| file | file | 是 | Excel文件（.xlsx），从知识星球后台导出 |
| planetId | string | 否 | 星球ID（默认使用配置的星球ID） |
| importType | string | 是 | 导入类型：full-全量覆盖/incremental-增量更新 |

**Excel文件格式要求**：
| 列名 | 类型 | 必填 | 说明 |
|------|------|------|------|
| 用户ID | string | 是 | 星球用户ID |
| 昵称 | string | 是 | 星球昵称 |
| 编号 | int | 是 | 星球会员编号 |
| 加入时间 | datetime | 是 | 加入星球时间 |

**请求示例**

```bash
curl -X POST /api/admin/members/sync \
  -H "Authorization: Bearer {token}" \
  -F "file=@planet_members.xlsx" \
  -F "planetId=28855511455142" \
  -F "importType=full"
```
...
```
GET /api/admin/checkins/sync/status/{taskId}
```
...
```json
{
  ...
  "message": "成功",
  "data": {
    "taskId": "sync_batch_1730000000",
    "status": "processing",
    "totalCamps": 5,
    "completedCamps": 3,
    "currentCamp": "21天早起打卡训练营",
    "progress": 60,
    "startTime": "2025-12-24T10:00:00",
    "endTime": null
  },
...
| requireBound | boolean | 否 | 是否要求已绑定（默认true，仅处理 bind_status=completed 的记录） |
| comment | string | 否 | 审核备注 |
...
```json
{
  ...
  "data": {
    ...
    "skippedDetails": [
      {
        "refundId": 5004,
        "reason": "bind_status=manual_review，需先手动绑定"
      }
    ],
    "auditor": "admin",
...
| 参数 | 类型 | 必填 | 说明 |
|------|------|------|------|
| type | string | 否 | 通知类型：success_notify/fail_alert |
| status | string | 否 | 发送状态：pending/sent/failed/skipped |
| page | int | 否 | 页码，默认1 |
| pageSize | int | 否 | 每页条数，默认20 |
    ...
    "list": [
      {
        ...
        "status": "sent",
        "retryCount": 0,
        "sendTime": "2025-12-31T10:00:00",
        "createdAt": "2025-12-31T09:55:00"
      }
    ],
    "total": 150,
    "page": 1,
...
| bindStatus | string | 绑定状态：completed |
| bindMethod | string | 绑定方式：h5_bindplanet |
...
sequenceDiagram
    ...
    H5->>API: GET /api/h5/orders/{orderNo}/params
    API-->>H5: 返回支付参数
    ...
    H5->>API: GET /api/h5/orders/{orderNo}/qrcode
...
flowchart TD
    A[训练营结束] --> B[系统生成退款名单]
    B --> C{bind_status?}
    C -->|completed| D[自动加入退款列表]
    C -->|pending| E{是否超过7天?}
    E -->|否| F[等待用户绑定]
    E -->|是| G[转人工审核]
    C -->|expired/manual_review| G
    F --> G
...
**场景描述**：用户通过固定二维码进入H5报名页面，填写星球信息后支付。此路径作为OAuth登录的降级方案，同时预留为星球API封锁后的备用入口。

**流程**：
1. 用户扫描固定二维码（URL格式：`https://h5.example.com/camps/{campId}/enroll`）
2. 进入H5报名页面，显示训练营信息
3. 用户填写星球ID和昵称（页面提供"如何获取星球ID"图例引导）
4. 调用 `POST /api/h5/payments` 创建订单
5. 支付成功后，系统自动完成绑定，`bind_status=completed, bind_method=user_fill`
6. 用户获取群二维码
...
| 微信OAuth授权 | 需要 | 不需要 |
| 星球信息填写 | 必填 | 必填（有图例引导） |
| bind_method | h5_bindplanet | user_fill |
| 适用场景 | 主路径，推荐使用 | 降级路径，API备用入口 |

**前端交互要求**：
- 必须在报名页面嵌入"如何获取星球ID"的图例演示
- 图例应展示：打开知识星球App → 个人中心 → 复制星球ID

---

## 13.3 非星球用户引导场景

**场景描述**：用户尚未加入知识星球，需要引导其先付费加入星球，再来报名训练营。

**流程**：
1. 用户进入H5报名页面
2. 用户不知道如何填写星球ID，点击"没有星球ID？"链接
3. 前端弹出星球分销海报（二维码/链接）
4. 提示用户：先扫码加入星球 → 加入成功后获取星球ID → 返回报名页面继续报名
5. 用户完成星球付费后，返回报名页面填写星球ID继续流程

**前端交互要求**：
- 报名页面需有"没有星球ID？点此加入"入口
- 点击后弹出模态框，展示：
  - 星球分销海报/二维码
  - 加入步骤说明
  - "已加入，去获取星球ID"按钮（跳转图例引导）
...
3. 对于已绑定（`bind_status=completed`）的申请，调用 `POST /admin/refunds/batch-approve` 批量通过
4. 对于待人工审核（`bind_status=manual_review`）的申请，先手动绑定后再审核
5. 系统自动执行退款并发送通知
...
| 版本 | 日期 | 变更内容 |
|------|------|----------|
| v1.1 | 2025-12-06 | 添加 SSOT 引用；修复 bind_status/bind_method 状态值与 SSOT 一致；移除已废弃的智能匹配和置信度概念；重写退款审核流程图 |

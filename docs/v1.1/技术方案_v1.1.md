...
| 版本 | 修订内容 | 日期 | 编制人 |
|------|----------|------|--------|
| v1.0 | 初版（基于传统模板） | 2025-10-24 | - |
| v1.1 | 重构为 AI 协作版，增加用户旅程、故事卡片、C4 架构 | 2025-11-29 | - |
...
8. [测试规格](#八测试规格)
9. [可观测性与运维](#九可观测性与运维)
10. [v1.1 vs v1 对比](#十v11-vs-v1-对比)
...
| S2.2 | 支付回调 | Webhook 3 秒内处理完成并保存映射 | P0 |
| S3.1 | 打卡同步 | 每日 01:00 自动同步，失败告警 | P0 |
| S4.1 | 自动匹配 | 数据库匹配置信度 100%，AI 匹配 ≥ 80% | P0 |
...
| 阶段 | 触点 | 用户行为 | 情绪 | 痛点/机会 |
|------|------|----------|------|-----------|
| 发现 | 知识星球/朋友圈 | 看到海报 | 期待 | 信息是否清晰 |
| 报名 | H5 页面 | 填写信息、支付 | 紧张 | 支付流程顺畅度 |
| 进群 | 微信群 | 扫码进群 | 兴奋 | 二维码有效期 |
| 打卡 | 知识星球 App | 每日打卡 | 坚持 | 打卡提醒 |
| 退款 | 微信通知 | 收到退款 | 满足 | 等待时间 |
...
| T2 | 支付回调 | 有效签名 | 保存映射，返回 success | P0 |
| T3 | 打卡同步 | 有效 Cookie | 同步成功，更新统计 | P0 |
| T4 | 数据库匹配 | 有映射记录 | 置信度 100% | P0 |
| T5 | 智能匹配 | 昵称相似 | 置信度 ≥ 80% | P0 |
| T6 | 执行退款 | 审核通过 | 调用 API 成功 | P0 |
...
- [PRD](../PRD.md) - 产品需求文档
- [v1 技术架构设计](../v1/技术架构设计.md) - 原版技术方案
- [v1 数据库设计](../v1/数据库设计.md) - 数据库设计

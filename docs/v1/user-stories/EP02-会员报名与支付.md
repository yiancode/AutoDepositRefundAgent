...
- [ ] Task 2.1.2: 实现OAuth回调处理 `GET /api/auth/callback/wechat-mp`
...
- **bind_method**：绑定完成时设置为 `h5_bindplanet`（OAuth主路径直接绑定）
- **幂等性保证**：订单号唯一索引
...
| **Story ID** | S2.6 |
| **角色** | 作为一名会员 |
| **目标** | 我希望能够通过固定二维码进入报名页面，填写星球信息后支付 |
| **价值** | 以便于在海报/宣传渠道快速报名，同时作为OAuth登录的降级方案和API封锁后的备用入口 |
| **优先级** | P1 |

### 验收标准

```gherkin
Feature: 固定二维码报名（先填后付）
  作为一名会员
  我希望能够通过固定二维码报名
  以便于在海报/宣传渠道快速报名
  ...
  Background:
    ...
    And 二维码URL格式为：https://h5.domain.com/camps/{campId}/enroll
    And 页面提供"如何获取星球ID"图例引导

  Scenario: 扫描固定二维码报名（已是星球用户）
    Given 用户扫描训练营固定二维码
    When 进入报名页面
    Then 显示训练营基本信息
    And 显示星球ID和昵称输入框
    And 显示"如何获取星球ID"图例引导入口
    When 用户填写星球ID和昵称
    And 点击支付按钮
    Then 创建订单（attach包含用户信息）
    And bind_status = "pending"
    And bind_method = "user_fill"
    When 支付成功
    Then bind_status更新为"completed"
    And 显示群二维码
  ...
  Scenario: 固定二维码 vs OAuth绑定
    ...
    When 选择固定二维码
    Then 无需微信OAuth授权
    And 必须手动填写星球信息（有图例引导）
    And bind_method = "user_fill"
    When 选择OAuth登录报名
    Then 需要微信OAuth授权
    And 必须手动填写星球信息
    And bind_method = "h5_bindplanet"
```
...
- **与OAuth绑定的区别**：
  ...
  | 微信OAuth授权 | 必需 | 不需要 |
  | 星球信息填写 | 必填 | 必填（有图例引导） |
  | attach参数 | 包含用户信息 | 包含用户信息 |
  | bind_method | h5_bindplanet | user_fill |
  | 适用场景 | 主路径，推荐 | 降级路径，API备用 |

### 前端交互要求

1. **星球ID获取图例**：
   - 在报名表单下方显示"如何获取星球ID？"链接
   - 点击后展开/弹出图文步骤说明
   - 建议使用截图+标注方式

2. **非星球用户引导**：
   - 在表单下方显示"没有星球ID？点此加入"链接
   - 点击后弹出模态框，包含：
     - 星球分销海报/二维码
     - 加入步骤说明文字
     - "已加入，去获取星球ID"按钮
...
- [ ] Task 2.6.2: 前端实现报名表单页面（必填星球信息）
- [ ] Task 2.6.3: 前端实现"如何获取星球ID"图例引导组件
- [ ] Task 2.6.4: 前端实现"非星球用户引导"模态框
- [ ] Task 2.6.5: 配置星球分销海报图片上传
- [ ] Task 2.6.6: 编写单元测试
- [ ] Task 2.6.7: 编写E2E测试
...
Feature: 订单状态查询
  ...
  Scenario: 查询已过期订单
    Given 支付后超过7天未绑定
    And bind_status = "expired" 或 "manual_review"
    When 调用查询接口
    Then 返回bind_status对应的状态值
    And 返回提示"绑定期限已过，管理员处理中"
    And 返回客服联系方式
...
| OAuth授权回调 | GET | `/api/auth/callback/wechat-mp` | 处理微信授权回调 | 无 |
| 创建支付订单 | POST | `/api/h5/payments` | 创建订单并返回支付参数 | 无 |
...
- [状态枚举定义](../design/状态枚举定义.md) - **SSOT** bind_status、bind_method 枚举定义
- [OAuth安全指南](../security/OAuth安全指南.md) - OAuth实现细节
...
### 决策2：bind_status 与 bind_method 对应关系
...
| `completed` | 已完成 | 用户绑定/人工审核通过 |
| `expired` | 已过期 | 超过7天未绑定 |
| `manual_review` | 待人工审核 | 超时后转入人工处理 |
| `closed` | 已关闭 | 人工审核拒绝 |

**bind_method 与绑定路径对应**

| bind_method | 含义 | 使用场景 | 对应Story |
|-------------|------|---------|----------|
| `h5_bindplanet` | H5绑定 | OAuth主路径：登录后直接绑定 | S2.2 OAuth绑定 |
| `user_fill` | 用户填写 | 降级路径：支付后补填星球信息 | S2.5, S2.6 |
| `manual` | 人工匹配 | 超时后管理员手动关联 | 人工审核 |
| NULL | 未绑定 | bind_status 非 completed 时 | - |

> **参考**: [状态枚举定义](../design/状态枚举定义.md)
...
**维护者**：产品 + 技术负责人
**变更历史**：
- 2025-12-05: v2.1 - 移除智能匹配方案，修正状态机图与SSOT一致，明确bind_method使用场景
- 2025-12-04: v2.0 - 初始版本

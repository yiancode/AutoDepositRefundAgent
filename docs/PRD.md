...
> **设计原则**：采用动态二维码 + 支付后绑定的混合方案，最大化自动匹配率，最小化人工干预。

### 7.1 四种匹配方式

| 匹配方式 | 覆盖率 | 准确率 | 实现方式 | 用户体验 |
|----------|--------|--------|----------|----------|
| **动态二维码** | 80% | 100% | 支付时 attach 携带 planet_user_id | 最佳（一步完成） |
| **用户填写** | 15% | 95% | 用户支付后 H5 页面填写星球信息 | 良好（两步完成） |
| **智能匹配** | 4% | 70% | Levenshtein 算法匹配昵称 | 无感知 |
| **人工绑定** | 1% | 100% | 管理员手动绑定 | 需等待 |

### 7.2 绑定状态机（bind_status）

```
[支付回调-有attach] → [completed] ← 动态二维码绑定
                          ↑
[支付回调-无attach] → [pending] → [completed] ← 用户填写绑定
                          ↓
                    [7天超时]
                          ↓
                 ...
                 [匹配失败]
                          ↓
               [manual_required] → [completed] ← 人工绑定
                          ↓
                 [无法匹配]
                          ↓
                     [closed]
...
| 状态 | 编码 | 描述 |
|------|------|------|
| 待绑定 | `pending` | 固定二维码支付后，等待用户填写 |
| 已绑定 | `completed` | 绑定成功（任意方式） |
| 已过期 | `expired` | 超过7天未绑定 |
| 需人工处理 | `manual_required` | 智能匹配失败 |
| 已关闭 | `closed` | 无法匹配，已关闭 |
...
| 方式 | 编码 | 触发场景 | 置信度 |
|------|------|----------|--------|
| 动态二维码 | `dynamic_qrcode` | 支付回调中 attach 包含用户信息 | 100% |
| 用户填写 | `user_fill` | 用户通过 H5 页面主动填写 | 95%（需人工确认） |
| 智能匹配 | `smart_match` | 绑定超时后系统自动匹配 | 按算法评分 |
| 人工绑定 | `manual` | 管理员手动绑定 | 100% |
...
for each 合格用户:
    ...
    switch (bind_status):
        case COMPLETED:
            // 直接使用已绑定映射
            置信度 = 100%（dynamic_qrcode/user_fill/manual）
                   或 算法评分（smart_match）
            加入退款名单
...
| 场景 | 触发条件 | 处理方式 |
|------|----------|----------|
| 绑定超时 | bind_status=pending 且超过7天 | 先尝试智能匹配，失败后人工处理 |
| 智能匹配失败 | 评分 < 50% | 管理员手动绑定 |
| 低置信度审核 | 评分 50-79% | 管理员确认后通过/拒绝 |
| 用户申诉 | 用户反馈匹配错误 | 管理员手动修正 |
...
| 术语 | 解释 |
|------|------|
| **动态二维码** | 包含用户身份信息（planet_user_id）的专属支付链接，支付时通过 attach 参数携带 |
| **支付映射** | 支付记录与知识星球用户的关联关系，存储在 payment_record 表 |
| **attach参数** | 企业微信支付时携带的自定义数据字段，用于传递 campId、planetUserId、orderNo |
| **匹配置信度** | 身份匹配的准确度百分比（100%/80%/50%等） |
...
| 状态 | 编码 | 描述 |
|------|------|------|
| 待绑定 | `pending` | 固定二维码支付后，等待用户填写星球信息 |
| 已绑定 | `completed` | 绑定成功（任意方式） |
| 已过期 | `expired` | 超过7天未绑定 |
| 需人工处理 | `manual_required` | 智能匹配失败，需管理员介入 |
| 已关闭 | `closed` | 无法匹配，已关闭 |
...
| 动态二维码 | `dynamic_qrcode` | 支付回调中 attach 包含用户信息 | 100% |
| 用户填写 | `user_fill` | 用户通过 H5 页面主动填写 | 95%（需人工确认） |
| 智能匹配 | `smart_match` | 绑定超时后系统自动匹配（Levenshtein算法） | 按算法评分 |
| 人工绑定 | `manual` | 管理员手动绑定 | 100% |
...
| **Levenshtein算法** | 计算两个字符串相似度的编辑距离算法，用于智能匹配 |

...
sequenceDiagram
    ...
    U->>ZSXQ: 点击报名链接
    ZSXQ->>API: 携带 planet_user_id
    API->>WX: 创建支付订单<br/>attach={planet_user_id}
    WX-->>API: 返回支付二维码URL
    API-->>U: 显示支付二维码

    U->>WX: 扫码支付
    WX->>API: Webhook回调<br/>包含 attach 参数
    API->>API: 解析 attach 获取 planet_user_id
    API->>DB: 保存支付映射
...
sequenceDiagram
    ...
    participant WX as 微信支付
    participant API as 后端API
    participant H5 as H5填写页面
    participant DB as SQLite

    U->>WX: 扫固定二维码支付
    WX->>API: Webhook回调
    API->>DB: 保存支付记录<br/>（planet_user_id=null）

    WX-->>U: 支付成功，跳转填写页面
    U->>H5: 打开填写页面
    H5->>U: 显示表单<br/>（星球ID、昵称）

    U->>H5: 填写信息并提交
    H5->>API: POST /api/payment/bind
    API->>DB: 更新支付映射<br/>（补充 planet_user_id）
...
详见 [混合方案技术设计.md](./混合方案技术设计.md) 中的智能匹配算法。
...
- [混合方案技术设计.md](./混合方案技术设计.md) - 完整技术方案
- [用户旅程图.md](./用户旅程图.md) - 用户流程图

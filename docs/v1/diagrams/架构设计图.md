...
> **文档版本**: v1.1
> **最后更新**: 2025-12-06
> **SSOT引用**: [状态枚举定义.md](../design/状态枚举定义.md) - 图中涉及的 CampStatus、PayStatus、BindStatus、RefundStatus 等枚举定义
...
C4Component
    ...
    Container_Boundary(api, "API 服务") {
        ...
        Component(zsxqClient, "ZsxqClient", "SDK", "知识星球 SDK（zsxq-sdk）")
...
| CheckinService | 打卡数据同步 | ZsxqClient, MemberMapper |
...
| 组件 | 职责 | 外部系统 |
|------|------|----------|
| ZsxqClient | 知识星球 SDK（zsxq-sdk） | 知识星球 |
...
> **设计说明**：移除人工审核流程，绑定超时检查直接触发通知和人工审核列表更新。v1 初期使用 Spring 异步任务，消息队列为可选扩展。

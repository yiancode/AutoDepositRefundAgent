...
|------|------|------|-------------|-----------|
| 已绑定（动态） | `COMPLETED` | 通过动态二维码自动绑定 | `dynamic_qrcode` | - |
| 已绑定（填写） | `COMPLETED` | 用户支付后填写绑定 | `user_fill` | - |
| 已绑定（匹配） | `COMPLETED` | 智能匹配自动绑定 | `smart_match` | - |
| 已绑定（人工） | `COMPLETED` | 管理员手动绑定 | `manual` | - |
| 待绑定 | `PENDING` | 等待用户填写 | `null` | 填写、等待超时 |
| 已过期 | `EXPIRED` | 超过绑定期限 | `null` | 智能匹配、人工处理 |
| 人工处理中 | `MANUAL_REQUIRED` | 智能匹配失败 | `null` | 人工绑定、关闭 |

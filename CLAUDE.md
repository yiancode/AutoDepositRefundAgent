...
详细的设计文档位于`docs/`目录：
- **产品需求文档（PRD.md）**：完整的功能需求和业务流程
- **技术架构设计（技术架构设计.md）**：技术选型、项目结构、接口设计
- **数据库设计（数据库设计.md）**：10张表的完整SQL和ER图
- **分阶段实施计划（分阶段实施计划.md）**：30天详细任务分解和验收标准
- **前端开发入门指南（前端开发入门指南.md）**：专为后端程序员设计的Vue学习路径
- **前端开发规范（前端开发规范.md）**：前端代码规范和最佳实践

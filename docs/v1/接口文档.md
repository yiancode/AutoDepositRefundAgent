...
| 参数 | 类型 | 必填 | 说明 |
|------|------|------|------|
| redirect_uri | string | 否 | 授权成功后的回调地址，默认为首页 |
| state | string | 否 | 自定义状态参数，防止CSRF攻击 |
...
| state | string | 用于回调验证的state参数 |
| expires_in | number | state有效期（秒） |
...
```json
{
  ...
  "message": "获取成功",
  "data": {
    "authorize_url": "https://open.weixin.qq.com/connect/oauth2/authorize?appid=wx123&redirect_uri=...",
    "state": "abc123xyz",
    "expires_in": 300
  },
  "timestamp": 1730000000
...
3. H5页面调用 `GET /api/h5/camps/1?planetUserId=123456789` 获取详情
4. 页面预填星球信息，用户确认后点击支付
5. 调用 `POST /api/h5/payments` 创建订单，请求体包含 planetUserId

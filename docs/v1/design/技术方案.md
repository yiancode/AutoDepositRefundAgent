...
| v1.0 | 整合所有文档为统一技术方案，发布版 | 2025-12-05 | - |
| v1.1 | 修复文档链接，添加 SSOT 引用，统一状态值大小写 | 2025-12-06 | - |
| v1.2 | 明确 FastAuth 文档引用关系 | 2025-12-08 | - |
| v1.3 | 更新5.1.0节用户报名流程图，优化为"先浏览再授权"，对齐H5原型 | 2025-12-12 | - |
| v1.4 | 新增5.6.4索引策略概要章节（架构验证补充） | 2025-12-12 | Winston |

> **关于 FastAuth 文档引用**：本技术方案中多处引用 [FastAuth接入方案.md](../archive/FastAuth接入方案.md) 的具体章节（如 §3.4、§2.5 等）。FastAuth 文档虽已归档，但仍为 OAuth 认证流程的详细设计参考，建议在实现相关功能时查阅。
...
4. [概要设计](#四概要设计)
5. [详细设计](#五详细设计)
   - [5.11 知识星球 SDK 集成](#511-知识星球-sdk-集成)
6. [风险与应对](#六风险与应对)
7. [测试方案](#七测试方案)
8. [指标监控](#八指标监控)
9. [上线方案](#九上线方案)
...
| 支付方式 | 覆盖率 | 准确率 | 实现方式 |
|----------|--------|--------|----------|
| H5 主路径 | 80% | 100% | 用户 OAuth 登录后绑定星球身份，直接支付 |
| 降级路径-用户填写 | 15% | 95% | 用户扫固定二维码支付后，H5 页面填写星球信息 |
| 人工处理 | 5% | 100% | 超时未绑定或信息异常由管理员人工绑定 |
...
| **OAuth认证** | 微信公众号 OAuth 2.0 | - | 用户身份识别（详见 [OAuth安全指南](../security/OAuth安全指南.md)） |
...
> **2025-12 更新**：流程优化为"先浏览，点报名再授权"，对齐 H5 原型设计。

```mermaid
flowchart TD
    A[用户访问 H5 链接] --> B[显示训练营列表<br/>无需登录]
    B --> C[用户点击某个训练营]
    C --> D[显示训练营详情<br/>无需登录]
    D --> E[用户点击"立即报名"]
    E --> F{检查本地登录态<br/>localStorage JWT}

    F -->|已登录且有效| G{星球是否已绑定?}
    F -->|未登录或已过期| H[跳转微信 OAuth 授权]
    H --> I[创建/获取 wechat_user]
    I --> J[生成 JWT Token]
    J --> G
...
| 路径 | 覆盖率 | 准确率 | 特点 |
|-----|--------|--------|------|
| **主路径（OAuth + 绑定）** | 80% | 100% | 推荐，OAuth 登录后绑定星球身份 |
| **降级路径（固定二维码）** | 15% | 95% | 扫固定码支付后手动填写星球信息 |
| **人工处理路径** | 5% | 100% | 超时未绑定由管理员处理 |

### 5.1.2 关键时刻 (Moments of Truth)

| 时刻 | 触发点 | 用户期望 | 设计对策 |
|------|--------|----------|----------|
| **MOT1: 支付** | 点击支付按钮 | 3秒内跳转 | 预加载支付参数 |
| **MOT2: 进群** | 支付成功后 | 立即看到二维码 | Webhook 实时处理 |
| **MOT3: 退款** | 训练营结束 | 24小时内收到 | 自动化退款流程 |

### 5.1.3 详细图表

> **📊 完整用户旅程图、UI 原型、情绪曲线** → 详见 [用户旅程图](../diagrams/用户旅程图.md)
...
| 流程 | 说明 | 关键节点 |
|-----|------|---------|
| **报名支付** | H5 主路径 vs 降级路径（互斥） | OAuth → 绑定 → 支付 → 显示群二维码 |
| **退款审核** | 仅已绑定用户自动退款，未绑定需人工处理 | 生成名单 → 审核 → 执行退款 |
| **绑定处理** | 用户主动绑定或管理员人工处理 | pending → completed/expired → manual_review |
| **打卡同步** | 定时从知识星球 API 同步 | API 获取 → 解析 → 宽限规则 → 合格判定 |
| **异常处理** | 支付/同步/退款三类异常 | 重试 → 告警 → 人工处理 |

### 5.2.3 详细流程图

> **📊 完整业务流程图（报名/退款/绑定/同步/异常处理）** → 详见 [业务流程图](../diagrams/业务流程图.md)
...
| 场景 | 参与者 | 关键步骤 |
|-----|--------|---------|
| **支付回调** | 微信支付→API→DB→H5 | 签名验证 → 幂等检查 → 解析attach → 绑定处理 |
| **固定码支付+绑定** | 会员→微信→API→H5 | 支付回调(无attach) → 保存pending → 用户填写 → 绑定完成 |
| **打卡同步** | Scheduler→API→知识星球 | 定时触发 → API调用 → 翻页处理 → 统计计算 |
| **退款执行** | Admin→API→MQ→退款服务→微信 | 批量审核 → 消息队列 → 调用API → 成功/重试 |
| **绑定超时** | Scheduler→API→Notify | 定时检查 → 更新expired → 转人工审核 |
| **Token过期** | Scheduler→API→Alert | 识别 TokenExpiredException → 暂停同步 → 告警通知 → 人工更新 |

### 5.3.2 详细时序图

> **📊 完整时序图（支付/绑定/同步/退款/异常）** → 详见 [时序图](../diagrams/时序图.md)
...
| 实体 | 状态流转 | 关键转换点 |
|-----|---------|-----------|
| **训练营** | 草稿→待发布→报名中→进行中→已结束→结算中→已归档 | 发布/开始/结束/结算 |
| **支付映射绑定** | 待绑定→已绑定 / 待绑定→已过期→人工审核→已关闭 | 用户填写/超时/人工处理 |
| **订单/支付** | 待支付→支付中→已支付→退款中→已退款 | 支付成功/发起退款/退款成功 |
| **退款** | 待审核→审核通过→待退款→退款中→退款成功 | 审核/执行/完成 |
| **打卡同步** | 待同步→同步中→同步成功/失败 | 定时触发/API调用 |
| **用户打卡** | 未开始→进行中→已完成/未达标→待退款→已退款 | 开始/达标判定/退款 |

### 5.4.2 绑定状态说明

> 状态值引用 [状态枚举定义.md](./状态枚举定义.md#1-bind_status---支付绑定状态)

| 状态 | 编码 | bind_method | 说明 |
|-----|------|-------------|------|
| 已绑定（H5） | `completed` | `h5_bindplanet` | 主路径 OAuth 支付 |
| 已绑定（填写） | `completed` | `user_fill` | 降级路径用户填写 |
| 已绑定（人工） | `completed` | `manual` | 管理员手动绑定 |
| 待绑定 | `pending` | - | 等待用户填写 |
| 已过期 | `expired` | - | 超过7天未绑定 |
| 人工审核中 | `manual_review` | - | 等待管理员处理 |
| 已关闭 | `closed` | - | 无法匹配 |

### 5.4.3 详细状态机图

> **📊 完整状态机图（训练营/支付/绑定/退款/同步）** → 详见 [状态机](../diagrams/状态机.md)
...
C4Component
    ...
    Container_Boundary(api, "API 服务") {
        ...
        Component(zsxqClient, "ZsxqClient", "SDK", "知识星球 SDK（详见 dev-zsxq-sdk集成指南）")
...
│   ├── CheckinService.java              # 打卡服务（打卡数据同步详见FastAuth接入方案§2.5）
│   ├── MatchService.java
│   ├── RefundService.java
│   └── impl/
├── manager/                                # 第三方API封装
...
├── config/                                 # 配置类
│   └── ZsxqSdkConfig.java               # 知识星球 SDK 配置（详见 dev-zsxq-sdk集成指南）
├── mapper/                                 # 数据访问层
│   ├── CampMapper.java
│   ├── MemberMapper.java
...
> 完整索引定义详见：📄 **[数据库设计.md](./数据库设计.md)** 第十一章

#### 索引设计原则

1. **软删除过滤**：所有业务索引添加 `WHERE deleted_at IS NULL` 部分索引，减少索引大小
2. **高频查询优先**：针对列表页、统计页等高频查询场景建立复合索引
3. **唯一约束索引**：使用部分唯一索引解决 NULL 值重复问题

#### 核心表索引清单

| 表名 | 索引名 | 字段 | 类型 | 用途 |
|------|--------|------|------|------|
| **training_camp** | idx_camp_status | status | 部分索引 | 按状态筛选训练营 |
| | idx_camp_dates | start_date, end_date | 部分索引 | 按日期范围查询 |
| | idx_camp_status_dates | status, start_date, end_date | 部分索引 | 训练营列表复合查询 |
| **camp_member** | uk_cm_camp_wechat | camp_id, wechat_user_id | 部分唯一索引 | 防止重复报名（主路径） |
| | uk_cm_camp_planet | camp_id, filled_planet_member_number | 部分唯一索引 | 防止重复报名（降级路径） |
| | idx_cm_camp_match | camp_id, match_status | 部分索引 | 会员列表筛选 |
| | idx_cm_refund_eligible | eligible_for_refund | 部分索引 | 退款资格筛选 |
| **payment_record** | idx_pr_order_no | order_no | 唯一索引 | 订单号查询 |
| | idx_pr_bind_status | bind_status | 部分索引 | 绑定状态筛选 |
| | idx_pr_bind_deadline | bind_deadline | 部分索引 | 超时绑定定时任务 |
| **refund_record** | uk_rr_payment | payment_id | 部分唯一索引 | 防止重复退款 |
| | idx_rr_pending_audit | camp_id, audit_status | 部分索引 | 待审核列表 |
| **checkin_record** | idx_cr_camp_date | camp_id, checkin_date | 普通索引 | 打卡统计查询 |
| **planet_user** | idx_pu_member_number | member_number | 部分索引 | 会员匹配查询 |
| | idx_pu_wechat_nickname | wechat_nickname | 部分索引 | 昵称模糊搜索 |
...
-- 4. 绑定超时检查（定时任务）
CREATE INDEX idx_pr_bind_deadline ON payment_record(bind_deadline)
...
| `REINDEX` | 每月 | 重建碎片化索引 |
| `ANALYZE` | 每周 | 更新统计信息 |
| `pg_stat_user_indexes` | 每周 | 监控索引使用率，清理无用索引 |
...
| POST | /api/h5/orders | 创建支付订单 | 无 |
| GET | /api/h5/orders/{orderNo}/params | 获取支付参数 | 无 |
| GET | /api/h5/orders/{orderNo}/status | 查询支付状态 | 无 |
| POST | /api/h5/orders/{orderNo}/bind-planet | 用户绑定星球信息 | **票据** |
| GET | /api/h5/orders/{orderNo}/qrcode | 获取群二维码 | **票据** |
| GET | /api/h5/progress/{memberId} | 查询打卡进度 | **票据** |

> **说明**：
> - 标注"票据"的接口需要在 Header 中传递 `X-Access-Token`
> - `/orders/{orderNo}/status` 无需票据，支付成功时响应中返回 `accessToken`
...
> **详细设计**：参见 [OAuth安全指南](../security/OAuth安全指南.md)
...
| 绑定超时检查 | `0 0 2 * * ?` | 每天 02:00 | 检查 `bind_status=pending` 且超时的记录，更新为 `expired`，转入人工审核流程（`manual_review`） |
...
| Token过期 | SDK 抛出 TokenExpiredException | 管理员 | 企微 | 知识星球 Token 已过期，请更新 |
...
public void processPolledPayment(WechatPayRecord record) {
    ...
    // 3. 发送绑定提醒通知（轮询补录的记录需用户自行绑定）
    notificationService.sendBindingReminder(payment);
...
> **集成指南**: 详见 [dev-zsxq-sdk集成指南.md](../guides/dev-zsxq-sdk集成指南.md)

### 5.11.1 SDK 概述

采用 zsxq-sdk Java 版本，提供类型安全的知识星球 API 调用，替代原有的直接 HTTP 调用方式。

| 属性 | 值 |
|------|-----|
| **SDK 仓库** | https://github.com/yiancode/zsxq-sdk |
| **语言版本** | Java 11+（兼容 Java 17） |
| **核心依赖** | OkHttp 4.12 + Gson 2.10 |
| **API 覆盖率** | 86.7%（65/75 接口） |
...
public class ZsxqSdkConfig {
    ...
    @Value("${zsxq.timeout:10000}")
    private int timeout;

    @Value("${zsxq.retry-count:3}")
...
```yaml
zsxq:
  token: ${ZSXQ_TOKEN:}           # 知识星球 Token（从浏览器 DevTools 获取）
  group-id: ${ZSXQ_GROUP_ID:}     # 星球 ID
...
| 业务需求 | SDK 方法 | 返回类型 | 说明 |
|----------|----------|----------|------|
| 获取打卡排行榜 | `checkins().getRankingList(groupId, checkinId)` | `List<RankingItem>` | 包含所有参与者的打卡天数 |
| 获取打卡统计 | `checkins().getStatistics(groupId, checkinId)` | `CheckinStatistics` | 总打卡数、参与人数 |
| 获取打卡项目列表 | `checkins().list(groupId)` | `List<Checkin>` | 支持 scope 过滤 |
| 获取星球信息 | `groups().get(groupId)` | `Group` | 星球名称、成员数 |
| 验证 Token 有效性 | `users().self()` | `User` | 检测 Token 是否过期 |

### 5.11.5 异常处理策略

SDK 提供分层异常体系，需在 Service 层统一处理：

```
ZsxqException (基类)
├── AuthException
│   ├── TokenInvalidException (错误码 10001) → 告警 + 人工更新 Token
│   └── TokenExpiredException (错误码 10002) → 告警 + 人工更新 Token
├── RateLimitException (错误码 40001)        → 等待后重试
└── NetworkException                          → 重试 3 次后告警
```
...
try {
    List<RankingItem> ranking = zsxqClient.checkins()
        .getRankingList(groupId, checkinId);
    // 处理打卡数据...
} catch (TokenExpiredException e) {
    // Token 过期，发送告警通知管理员更新
    notificationService.sendAlert("知识星球 Token 已过期，请更新 system_config");
    syncLog.setStatus(SyncStatus.FAILED);
    syncLog.setErrorMessage("Token 已过期 (错误码: " + e.getCode() + ")");
    ...
    // 其他 API 错误
    log.error("知识星球 API 调用失败: code={}, msg={}", e.getCode(), e.getMessage());
}
```
...
| SDK 字段 | 数据库字段 | 表 | 说明 |
|----------|------------|-----|------|
| `RankingItem.user.userId` | `planet_user_id` | `checkin_record` | 用户在星球的唯一 ID |
| `RankingItem.user.name` | `nickname` | `planet_user` | 用于匹配验证 |
| `RankingItem.count` | `checkin_count` | `camp_member` | 累计打卡天数 |
| `RankingItem.rank` | - | - | 排名（仅展示用，不存储） |
...
sequenceDiagram
    ...
    participant Service as CheckinSyncService
    participant SDK as ZsxqClient
    participant DB as PostgreSQL
    participant Alert as 告警服务

    Task->>Service: 触发同步 (每日 01:00)
    Service->>SDK: checkins().getRankingList()

    alt Token 有效
        SDK-->>Service: List<RankingItem>
        loop 遍历排行榜
            Service->>DB: INSERT checkin_record
            Service->>DB: UPDATE camp_member.checkin_count
        end
        Service->>DB: INSERT sync_log (SUCCESS)
    else Token 过期
        SDK-->>Service: TokenExpiredException
        Service->>Alert: 发送告警通知
        Service->>DB: INSERT sync_log (FAILED)
    end
```
...
| 知识星球 Token 过期 | 高 | 高 | SDK 抛出 TokenExpiredException，自动告警通知更新 |
...
>
> 📄 **[AI辅助敏捷开发计划.md](../guides/AI辅助敏捷开发计划.md)**

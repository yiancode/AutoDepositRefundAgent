...
> 使用微信商户平台的 JSAPI 支付，适用于微信内H5页面

#### 6.1.1 支付流程
1. H5页面通过微信公众号OAuth获取用户OpenID
2. 前端调用后端API，后端调用微信支付统一下单接口
3. 前端使用返回的支付参数调起微信支付
4. 支付成功后，微信通过Webhook回调后端
5. 后端记录支付数据，前端轮询或WebSocket获取结果

#### 6.1.2 Webhook处理
- **接收参数**：订单号、支付金额、OpenID、支付时间
- **处理逻辑**：
  1. 验证签名（微信支付v3签名）
  2. 查询订单是否存在
  3. 更新支付状态
...
- 微信OAuth获取OpenID成功
- 微信支付下单和回调正常工作
- H5 绑定星球账号后：bind_status=completed, bind_method=h5_bindplanet
- 用户绑定成功后返回群二维码
...
| 术语 | 解释 |
|------|------|
| **OpenID** | 用户在该公众号下的唯一标识，用于微信支付下单，28位字符串 |
| **UnionID** | 同一开放平台下多个应用的统一标识（需绑定开放平台才能获取） |
| **JSAPI支付** | 微信内H5页面调起微信支付的方式，需要用户OpenID |
| **微信商户平台** | 微信支付的商户管理后台（pay.weixin.qq.com） |
| **服务号** | 微信公众号的一种类型，支持OAuth获取用户OpenID |
...
| 术语 | 解释 |
|------|------|
| **Webhook** | 微信支付结果的异步回调通知 |
| **幂等处理** | 确保同一请求多次执行结果一致，防止重复处理 |
| **分布式锁** | Redis 实现的并发控制机制，防止定时任务重复执行 |
| **OAuth** | 开放授权协议，用于获取用户身份（微信OpenID、知识星球用户身份） |

### 附录B：参考资料
- [微信支付JSAPI文档](https://pay.weixin.qq.com/wiki/doc/apiv3/open/pay/chapter1_1_1.shtml)
- [微信公众号OAuth文档](https://developers.weixin.qq.com/doc/offiaccount/OA_Web_Apps/Wechat_webpage_authorization.html)
- 知识星球API文档（待补充）
- Spring Boot官方文档
- PostgreSQL官方文档

...
#### `/evaluate-next` - 评估下一个待评估文档
**用途**: 自动评估 `dev-文档评估指南.md` 中下一个待评估的文档，检查与 SSOT 的一致性。

**自动执行**:
1. 读取评估指南，找到下一个 `⬜` 状态的文档
2. 读取 SSOT (`状态枚举定义.md`) 作为标准
3. 执行一致性检查
4. 修复发现的问题
5. 更新评估指南状态
...
**检查项**:
- 状态枚举一致性 (bind_status, pay_status, camp_status 等)
- 已废弃概念 (智能匹配、置信度、FastAuth)
- 文档链接有效性
- 状态机图正确性
...
#### `/evaluate-batch` - 批量评估所有文档
**用途**: 连续评估所有待评估文档，适合一次性完成评估任务。

**注意**: 每评估 2-3 个文档后建议运行 `/compact` 压缩上下文。

**推荐工作流**:
```
/evaluate-next    # 评估一个文档
/compact          # 压缩上下文
/evaluate-next    # 继续下一个
```

#### `/evaluate-parallel` - 并行评估多个文档
**用途**: 使用多个子代理并行评估 3-4 个文档，加速评估进度。

**工作原理**:
1. 并行启动多个 Task 子代理，每个评估一个文档
2. 子代理只评估不修改，返回评估报告
3. 主代理汇总结果后串行修复（避免冲突）
4. 一次性更新评估指南并提交
...
#### `@doc-evaluator` - 文档评估代理
**用途**: 专门用于文档评估的代理，具有只读 + 编辑权限，不执行危险命令。

**调用方式**:
```
@doc-evaluator 评估 EP03-打卡数据同步.md
```
...
- [前端开发规范](../../docs/archive/v0/4前端开发规范.md) - 前端规范
- [后端开发规范](../../docs/archive/v0/5后端开发规范.md) - 后端规范

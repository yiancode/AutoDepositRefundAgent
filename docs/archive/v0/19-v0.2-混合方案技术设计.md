# v0.2 混合方案技术设计

## 📋 目录

- [方案概述](#方案概述)
- [技术架构](#技术架构)
- [核心模块设计](#核心模块设计)
- [API 接口设计](#api-接口设计)
- [智能匹配算法](#智能匹配算法)
- [错误处理与容错](#错误处理与容错)
- [性能优化](#性能优化)
- [安全设计](#安全设计)

---

## 方案概述

### 核心问题

如何确保**数据流（知识星球打卡）** 和 **资金流（微信支付订单）** 的一致性？

### 解决方案

**混合方案** = SQLite 存储 + 智能匹配算法 + 人工审核

```
用户报名支付 → SQLite 存储映射 → 打卡完成 → 查询映射生成退款
                     ↓ 如果缺失
                 智能匹配算法补充
                     ↓ 如果不确定
                   人工审核确认
```

### 方案优势

| 特性 | 说明 |
|------|------|
| **准确性** | 数据库记录 100% 准确 |
| **自动化** | 90%+ 自动处理，无需人工 |
| **容错性** | 多层兜底机制，支持人工介入 |
| **轻量级** | 使用 SQLite，无需独立数据库服务 |
| **可扩展** | 易于迁移到 v1 的 PostgreSQL |

---

## 技术架构

### 系统架构图

```
┌─────────────────────────────────────────────────────────────┐
│                        H5 前端                               │
│              (Vue 3 + Vite + Element Plus)                   │
└─────────────────────────────────────────────────────────────┘
                            ↓ HTTP/HTTPS
┌─────────────────────────────────────────────────────────────┐
│                      Express.js API                          │
│  ┌──────────────┬──────────────┬──────────────────────┐    │
│  │ Routes       │ Services     │ Utils                 │    │
│  │ - camps      │ - zsxq       │ - logger              │    │
│  │ - payment    │ - refund     │ - response            │    │
│  │ - refund     │ - payment    │ - sanitize            │    │
│  │              │ - matching   │                       │    │
│  └──────────────┴──────────────┴──────────────────────┘    │
└─────────────────────────────────────────────────────────────┘
                            ↓
┌─────────────────────────────────────────────────────────────┐
│                   数据层 & 外部 API                          │
│  ┌──────────────┬──────────────┬──────────────────────┐    │
│  │ SQLite       │ 知识星球API   │ 微信支付API           │    │
│  │ (映射存储)   │ (打卡数据)    │ (交易账单)            │    │
│  └──────────────┴──────────────┴──────────────────────┘    │
└─────────────────────────────────────────────────────────────┘
```

### 技术栈

| 层级 | 技术 | 版本 | 说明 |
|------|------|------|------|
| **数据库** | SQLite 3 | - | 轻量级关系型数据库 |
| **ORM** | better-sqlite3 | ^9.0.0 | 高性能同步 SQLite 库 |
| **HTTP 客户端** | axios | ^1.6.0 | 调用外部 API |
| **加密/签名** | crypto (Node.js) | - | 微信支付签名 |
| **日志** | winston | ^3.11.0 | 日志记录 |
| **测试** | jest | ^30.2.0 | 单元测试 & 集成测试 |

### 目录结构

```
zsxq-api/
├── src/
│   ├── services/
│   │   ├── payment-mapping.service.js    # 支付映射存储
│   │   ├── smart-matching.service.js     # 智能匹配算法
│   │   ├── wechat-payment.service.js     # 微信支付 API
│   │   ├── refund-file.service.js        # 退款文件生成
│   │   └── ...
│   ├── routes/
│   │   ├── payment.js                    # 支付相关路由
│   │   └── refund.js                     # 退款相关路由
│   ├── utils/
│   │   ├── wechat-auth.js                # 微信支付签名
│   │   └── csv-generator.js              # CSV 文件生成
│   └── db/
│       ├── init.js                       # 数据库初始化
│       └── migrations/                   # 数据库迁移脚本
├── data/
│   └── payment-mapping.db                # SQLite 数据库文件
├── temp/
│   └── refund-*.csv                      # 临时生成的 CSV 文件
└── tests/
    ├── unit/
    │   ├── payment-mapping.test.js
    │   ├── smart-matching.test.js
    │   └── ...
    └── integration/
        ├── refund-flow.test.js
        └── ...
```

---

## 核心模块设计

### 1. 支付映射存储模块

**文件**: `src/services/payment-mapping.service.js`

```javascript
const Database = require('better-sqlite3');
const logger = require('../utils/logger');

class PaymentMappingService {
  constructor() {
    this.db = new Database('data/payment-mapping.db');
    this.initDatabase();
  }

  /**
   * 初始化数据库表
   */
  initDatabase() {
    this.db.exec(`
      CREATE TABLE IF NOT EXISTS payment_mapping (
        id INTEGER PRIMARY KEY AUTOINCREMENT,
        out_trade_no TEXT UNIQUE NOT NULL,
        transaction_id TEXT,
        planet_user_id TEXT,
        planet_nickname TEXT,
        wechat_nickname TEXT,
        amount INTEGER NOT NULL,
        checkin_id TEXT NOT NULL,
        payment_time TEXT NOT NULL,
        created_at DATETIME DEFAULT CURRENT_TIMESTAMP,
        updated_at DATETIME DEFAULT CURRENT_TIMESTAMP
      );

      CREATE INDEX IF NOT EXISTS idx_planet_user
        ON payment_mapping(planet_user_id, checkin_id);

      CREATE INDEX IF NOT EXISTS idx_checkin
        ON payment_mapping(checkin_id);

      CREATE INDEX IF NOT EXISTS idx_payment_time
        ON payment_mapping(payment_time);
    `);
  }

  /**
   * 保存支付映射关系
   */
  saveMapping(data) {
    const stmt = this.db.prepare(`
      INSERT INTO payment_mapping
      (out_trade_no, transaction_id, planet_user_id, planet_nickname,
       wechat_nickname, amount, checkin_id, payment_time)
      VALUES (?, ?, ?, ?, ?, ?, ?, ?)
      ON CONFLICT(out_trade_no) DO UPDATE SET
        transaction_id = excluded.transaction_id,
        planet_user_id = excluded.planet_user_id,
        planet_nickname = excluded.planet_nickname,
        wechat_nickname = excluded.wechat_nickname,
        updated_at = CURRENT_TIMESTAMP
    `);

    try {
      const result = stmt.run(
        data.out_trade_no,
        data.transaction_id,
        data.planet_user_id,
        data.planet_nickname,
        data.wechat_nickname,
        data.amount,
        data.checkin_id,
        data.payment_time
      );

      logger.info('支付映射保存成功', {
        out_trade_no: data.out_trade_no,
        planet_user_id: data.planet_user_id
      });

      return result;
    } catch (error) {
      logger.error('支付映射保存失败', error);
      throw error;
    }
  }

  /**
   * 根据知识星球用户查询支付记录
   */
  getByPlanetUser(planetUserId, checkinId) {
    const stmt = this.db.prepare(`
      SELECT * FROM payment_mapping
      WHERE planet_user_id = ? AND checkin_id = ?
    `);

    return stmt.get(planetUserId, checkinId);
  }

  /**
   * 根据训练营 ID 批量查询
   */
  getByCheckin(checkinId) {
    const stmt = this.db.prepare(`
      SELECT * FROM payment_mapping
      WHERE checkin_id = ?
      ORDER BY payment_time DESC
    `);

    return stmt.all(checkinId);
  }

  /**
   * 统计某个训练营的支付记录数
   */
  countByCheckin(checkinId) {
    const stmt = this.db.prepare(`
      SELECT COUNT(*) as count
      FROM payment_mapping
      WHERE checkin_id = ?
    `);

    return stmt.get(checkinId).count;
  }
}

module.exports = new PaymentMappingService();
```

### 2. 微信支付 API 模块

**文件**: `src/services/wechat-payment.service.js`

```javascript
const axios = require('axios');
const crypto = require('crypto');
const fs = require('fs');
const logger = require('../utils/logger');

class WechatPaymentService {
  constructor() {
    this.mchid = process.env.WECHAT_PAY_MCHID;
    this.apiKey = process.env.WECHAT_PAY_API_KEY;
    this.serialNo = process.env.WECHAT_PAY_SERIAL_NO;

    // 加载证书
    this.privateKey = fs.readFileSync(
      process.env.WECHAT_PAY_KEY_PATH,
      'utf8'
    );
  }

  /**
   * 生成请求签名
   */
  generateSignature(method, url, timestamp, nonce, body = '') {
    const message = `${method}\n${url}\n${timestamp}\n${nonce}\n${body}\n`;

    const sign = crypto.createSign('RSA-SHA256');
    sign.update(message);

    return sign.sign(this.privateKey, 'base64');
  }

  /**
   * 生成 Authorization 头
   */
  generateAuth(method, url, body = '') {
    const timestamp = Math.floor(Date.now() / 1000);
    const nonce = crypto.randomBytes(16).toString('hex');
    const signature = this.generateSignature(
      method,
      url,
      timestamp,
      nonce,
      body
    );

    return `WECHATPAY2-SHA256-RSA2048 ` +
      `mchid="${this.mchid}",` +
      `nonce_str="${nonce}",` +
      `signature="${signature}",` +
      `timestamp="${timestamp}",` +
      `serial_no="${this.serialNo}"`;
  }

  /**
   * 下载交易账单
   */
  async getTradeBill(billDate, billType = 'ALL') {
    const url = '/v3/bill/tradebill';
    const fullUrl = `https://api.mch.weixin.qq.com${url}`;

    try {
      // 1. 获取账单下载链接
      const response = await axios.get(fullUrl, {
        params: {
          bill_date: billDate,
          bill_type: billType
        },
        headers: {
          'Authorization': this.generateAuth('GET', url),
          'Accept': 'application/json'
        }
      });

      const downloadUrl = response.data.download_url;

      // 2. 下载账单文件
      const billData = await axios.get(downloadUrl, {
        headers: {
          'Authorization': this.generateAuth('GET', downloadUrl)
        }
      });

      logger.info('交易账单下载成功', { billDate, billType });

      // 3. 解析 CSV 数据
      return this.parseBillData(billData.data);
    } catch (error) {
      logger.error('交易账单下载失败', error);
      throw error;
    }
  }

  /**
   * 解析账单 CSV 数据
   */
  parseBillData(csvData) {
    const lines = csvData.split('\n');
    const payments = [];

    // 跳过前几行的说明文字
    let dataStartIndex = 0;
    for (let i = 0; i < lines.length; i++) {
      if (lines[i].startsWith('交易时间')) {
        dataStartIndex = i + 1;
        break;
      }
    }

    // 解析数据行
    for (let i = dataStartIndex; i < lines.length; i++) {
      const line = lines[i].trim();
      if (!line || line.startsWith('总交易单数')) break;

      const fields = line.split(',').map(f => f.trim().replace(/`/g, ''));

      if (fields.length >= 10) {
        payments.push({
          payment_time: fields[0],
          out_trade_no: fields[2],
          transaction_id: fields[3],
          payer_name: fields[8],
          amount: parseFloat(fields[9]),
          trade_state: fields[4]
        });
      }
    }

    return payments;
  }

  /**
   * 查询单笔订单
   */
  async queryOrder(outTradeNo) {
    const url = `/v3/pay/transactions/out-trade-no/${outTradeNo}`;
    const fullUrl = `https://api.mch.weixin.qq.com${url}`;

    try {
      const response = await axios.get(fullUrl, {
        params: {
          mchid: this.mchid
        },
        headers: {
          'Authorization': this.generateAuth('GET', url),
          'Accept': 'application/json'
        }
      });

      return response.data;
    } catch (error) {
      logger.error('订单查询失败', { outTradeNo, error });
      throw error;
    }
  }
}

module.exports = new WechatPaymentService();
```

### 3. 智能匹配算法模块

**文件**: `src/services/smart-matching.service.js`

```javascript
const logger = require('../utils/logger');
const wechatPaymentService = require('./wechat-payment.service');

class SmartMatchingService {
  /**
   * 智能匹配：知识星球用户 → 微信支付订单
   */
  async matchUsers(users, checkinId) {
    // 1. 获取训练营开始日期的账单
    const billDate = this.getCheckinStartDate(checkinId);
    const payments = await wechatPaymentService.getTradeBill(billDate);

    // 2. 过滤出本训练营的订单（根据金额）
    const checkinAmount = this.getCheckinAmount(checkinId);
    const filteredPayments = payments.filter(p =>
      p.amount === checkinAmount &&
      p.trade_state === 'SUCCESS'
    );

    logger.info('可用支付记录', {
      total: payments.length,
      filtered: filteredPayments.length
    });

    // 3. 逐个用户进行匹配
    const matched = [];
    const unmatched = [];

    for (const user of users) {
      const result = this.findBestMatch(user, filteredPayments);

      if (result && result.confidence >= 50) {
        matched.push({
          ...user,
          ...result,
          match_method: 'smart_matching'
        });
      } else {
        unmatched.push(user);
      }
    }

    return { matched, unmatched };
  }

  /**
   * 为单个用户找到最佳匹配
   */
  findBestMatch(user, payments) {
    let bestMatch = null;
    let highestScore = 0;

    for (const payment of payments) {
      const score = this.calculateMatchScore(user, payment);

      if (score > highestScore) {
        highestScore = score;
        bestMatch = {
          out_trade_no: payment.out_trade_no,
          transaction_id: payment.transaction_id,
          amount: payment.amount,
          payment_time: payment.payment_time,
          confidence: score,
          payer_name: payment.payer_name
        };
      }
    }

    return highestScore >= 50 ? bestMatch : null;
  }

  /**
   * 计算匹配评分（0-100）
   */
  calculateMatchScore(user, payment) {
    let score = 0;

    // 1. 昵称匹配（权重 60%）
    const nicknameScore = this.compareNicknames(
      user.planet_nickname,
      payment.payer_name
    );
    score += nicknameScore * 0.6;

    // 2. 时间匹配（权重 20%）
    const timeScore = this.compareTime(
      user.join_time,
      payment.payment_time
    );
    score += timeScore * 0.2;

    // 3. 金额匹配（权重 20%）
    const amountScore = this.compareAmount(
      this.getCheckinAmount(user.checkin_id),
      payment.amount
    );
    score += amountScore * 0.2;

    return Math.round(score);
  }

  /**
   * 昵称相似度比较（0-100）
   */
  compareNicknames(nickname1, nickname2) {
    if (!nickname1 || !nickname2) return 0;

    // 完全匹配
    if (nickname1 === nickname2) return 100;

    // 包含关系
    if (nickname1.includes(nickname2) || nickname2.includes(nickname1)) {
      return 80;
    }

    // 移除空格后比较
    const clean1 = nickname1.replace(/\s/g, '');
    const clean2 = nickname2.replace(/\s/g, '');

    if (clean1 === clean2) return 90;
    if (clean1.includes(clean2) || clean2.includes(clean1)) {
      return 70;
    }

    // Levenshtein 距离算法
    const distance = this.levenshteinDistance(clean1, clean2);
    const maxLen = Math.max(clean1.length, clean2.length);
    const similarity = (1 - distance / maxLen) * 100;

    return Math.max(0, Math.round(similarity));
  }

  /**
   * Levenshtein 距离算法
   */
  levenshteinDistance(str1, str2) {
    const matrix = [];

    for (let i = 0; i <= str2.length; i++) {
      matrix[i] = [i];
    }

    for (let j = 0; j <= str1.length; j++) {
      matrix[0][j] = j;
    }

    for (let i = 1; i <= str2.length; i++) {
      for (let j = 1; j <= str1.length; j++) {
        if (str2.charAt(i - 1) === str1.charAt(j - 1)) {
          matrix[i][j] = matrix[i - 1][j - 1];
        } else {
          matrix[i][j] = Math.min(
            matrix[i - 1][j - 1] + 1,
            matrix[i][j - 1] + 1,
            matrix[i - 1][j] + 1
          );
        }
      }
    }

    return matrix[str2.length][str1.length];
  }

  /**
   * 时间相似度比较（0-100）
   */
  compareTime(joinTime, paymentTime) {
    if (!joinTime || !paymentTime) return 50;  // 默认分数

    const join = new Date(joinTime);
    const payment = new Date(paymentTime);

    const diffHours = Math.abs(join - payment) / (1000 * 60 * 60);

    // 支付时间应该在加入时间前后 24 小时内
    if (diffHours <= 1) return 100;
    if (diffHours <= 6) return 80;
    if (diffHours <= 24) return 60;
    if (diffHours <= 72) return 40;

    return 0;
  }

  /**
   * 金额匹配（0-100）
   */
  compareAmount(expectedAmount, actualAmount) {
    return expectedAmount === actualAmount ? 100 : 0;
  }

  /**
   * 获取训练营开始日期
   */
  getCheckinStartDate(checkinId) {
    // TODO: 从知识星球 API 获取训练营开始时间
    // 这里暂时返回当前日期
    return new Date().toISOString().split('T')[0];
  }

  /**
   * 获取训练营押金金额
   */
  getCheckinAmount(checkinId) {
    // TODO: 从配置或 API 获取
    return 99.00;  // 默认 99 元
  }
}

module.exports = new SmartMatchingService();
```

### 4. 退款文件生成模块

**文件**: `src/services/refund-file.service.js`

```javascript
const path = require('path');
const fs = require('fs');
const logger = require('../utils/logger');
const refundService = require('./refund.service');
const paymentMappingService = require('./payment-mapping.service');
const smartMatchingService = require('./smart-matching.service');

class RefundFileService {
  /**
   * 生成退款文件（主入口）
   */
  async generateRefundFile(checkinId, requiredDays) {
    logger.info('开始生成退款文件', { checkinId, requiredDays });

    // 1. 从知识星球获取合格用户列表
    const refundList = await refundService.generateRefundList(
      checkinId,
      requiredDays
    );

    const qualifiedUsers = refundList.refund_list.filter(
      u => u.is_qualified
    );

    // 2. 尝试从数据库匹配
    const { dbMatched, dbUnmatched } = await this.matchFromDatabase(
      qualifiedUsers,
      checkinId
    );

    // 3. 对未匹配的用户，尝试智能匹配
    let aiMatched = [];
    if (dbUnmatched.length > 0) {
      const result = await smartMatchingService.matchUsers(
        dbUnmatched,
        checkinId
      );
      aiMatched = result.matched;
    }

    // 4. 分类结果
    const allMatched = [...dbMatched, ...aiMatched];
    const needReview = aiMatched.filter(m => m.confidence < 80);
    const autoApproved = allMatched.filter(m => m.confidence >= 80);
    const failed = qualifiedUsers.filter(
      u => !allMatched.find(m => m.planet_user_id === u.planet_user_id)
    );

    // 5. 生成 CSV 文件
    const csvContent = this.generateCSV(autoApproved);
    const filename = `refund_${checkinId}_${Date.now()}.csv`;
    const filepath = path.join(__dirname, '../../temp', filename);

    // 确保 temp 目录存在
    if (!fs.existsSync(path.dirname(filepath))) {
      fs.mkdirSync(path.dirname(filepath), { recursive: true });
    }

    fs.writeFileSync(filepath, csvContent, 'utf8');

    logger.info('退款文件生成成功', {
      filename,
      total: qualifiedUsers.length,
      auto_matched: dbMatched.length,
      ai_matched: aiMatched.length,
      need_review: needReview.length,
      failed: failed.length
    });

    // 6. 返回结果
    return {
      csv_filename: filename,
      csv_url: `/downloads/${filename}`,
      statistics: {
        total_qualified: qualifiedUsers.length,
        auto_matched: dbMatched.length,
        ai_matched: aiMatched.length,
        need_manual_review: needReview.length,
        failed: failed.length
      },
      need_review: needReview.map(m => ({
        planet_user_id: m.planet_user_id,
        planet_nickname: m.planet_nickname,
        out_trade_no: m.out_trade_no,
        payer_name: m.payer_name,
        confidence: m.confidence,
        reason: this.getMatchReason(m.confidence)
      })),
      failed: failed.map(u => ({
        planet_user_id: u.planet_user_id,
        planet_nickname: u.planet_nickname,
        reason: '未找到匹配的支付记录'
      }))
    };
  }

  /**
   * 从数据库匹配
   */
  async matchFromDatabase(users, checkinId) {
    const dbMatched = [];
    const dbUnmatched = [];

    for (const user of users) {
      const payment = paymentMappingService.getByPlanetUser(
        user.planet_user_id,
        checkinId
      );

      if (payment) {
        dbMatched.push({
          ...user,
          out_trade_no: payment.out_trade_no,
          transaction_id: payment.transaction_id,
          amount: payment.amount / 100,  // 分 → 元
          confidence: 100,
          match_method: 'database'
        });
      } else {
        dbUnmatched.push(user);
      }
    }

    return { dbMatched, dbUnmatched };
  }

  /**
   * 生成 CSV 内容
   */
  generateCSV(matchedList) {
    let csv = '';

    for (const item of matchedList) {
      const refundNo = `REFUND_${item.out_trade_no}`;
      const reason = '完成打卡';

      // 格式：商户订单号,退款金额,退款原因,商户退款单号
      csv += `${item.out_trade_no},${item.amount.toFixed(2)},${reason},${refundNo}\n`;
    }

    return csv;
  }

  /**
   * 获取匹配原因说明
   */
  getMatchReason(confidence) {
    if (confidence >= 80) return 'AI 高置信度匹配';
    if (confidence >= 60) return '昵称部分匹配';
    if (confidence >= 50) return '低置信度匹配';
    return '无法匹配';
  }
}

module.exports = new RefundFileService();
```

---

## API 接口设计

### 路由定义

**文件**: `src/routes/refund.js`

```javascript
const express = require('express');
const router = express.Router();
const refundFileService = require('../services/refund-file.service');
const { success, error } = require('../utils/response');
const logger = require('../utils/logger');

/**
 * @swagger
 * /api/camps/{checkinId}/refund-file:
 *   post:
 *     summary: 生成退款文件
 *     tags: [退款管理]
 *     parameters:
 *       - in: path
 *         name: checkinId
 *         required: true
 *         schema:
 *           type: string
 *     requestBody:
 *       required: true
 *       content:
 *         application/json:
 *           schema:
 *             type: object
 *             properties:
 *               required_days:
 *                 type: integer
 *                 description: 完成要求天数
 *                 example: 7
 *     responses:
 *       200:
 *         description: 退款文件生成成功
 */
router.post('/:checkinId/refund-file', async (req, res, next) => {
  try {
    const { checkinId } = req.params;
    const { required_days } = req.body;

    // 参数校验
    if (!checkinId) {
      return res.status(400).json(error('训练营 ID 不能为空', 400));
    }

    if (!required_days || required_days < 1) {
      return res.status(400).json(error('完成要求天数必须大于 0', 400));
    }

    // 生成退款文件
    const result = await refundFileService.generateRefundFile(
      checkinId,
      required_days
    );

    res.json(success(result, '退款文件生成成功'));
  } catch (err) {
    logger.error('生成退款文件失败', err);
    next(err);
  }
});

/**
 * @swagger
 * /api/refund/review:
 *   post:
 *     summary: 人工审核确认匹配
 *     tags: [退款管理]
 */
router.post('/review', async (req, res, next) => {
  try {
    const { planet_user_id, out_trade_no, action } = req.body;

    // TODO: 实现人工审核逻辑
    // 1. 保存审核结果到数据库
    // 2. 如果通过，更新匹配关系
    // 3. 如果拒绝，标记为需人工处理

    res.json(success({ action }, '审核成功'));
  } catch (err) {
    logger.error('审核失败', err);
    next(err);
  }
});

module.exports = router;
```

**文件**: `src/routes/payment.js`

```javascript
const express = require('express');
const router = express.Router();
const paymentMappingService = require('../services/payment-mapping.service');
const { success, error } = require('../utils/response');
const logger = require('../utils/logger');

/**
 * @swagger
 * /api/payment/mapping:
 *   post:
 *     summary: 保存支付映射关系
 *     tags: [支付管理]
 */
router.post('/mapping', async (req, res, next) => {
  try {
    const data = req.body;

    // 参数校验
    if (!data.out_trade_no || !data.checkin_id || !data.amount) {
      return res.status(400).json(error('缺少必需参数', 400));
    }

    // 保存映射
    paymentMappingService.saveMapping(data);

    res.json(success({}, '支付映射保存成功'));
  } catch (err) {
    logger.error('保存支付映射失败', err);
    next(err);
  }
});

/**
 * @swagger
 * /api/payment/webhook:
 *   post:
 *     summary: 微信支付回调
 *     tags: [支付管理]
 */
router.post('/webhook', async (req, res, next) => {
  try {
    // TODO: 实现微信支付回调处理
    // 1. 验证签名
    // 2. 解析支付结果
    // 3. 保存支付映射
    // 4. 返回群二维码

    res.json({ code: 'SUCCESS', message: '成功' });
  } catch (err) {
    logger.error('支付回调处理失败', err);
    res.json({ code: 'FAIL', message: '失败' });
  }
});

module.exports = router;
```

### 下载文件路由

**文件**: `src/routes/downloads.js`

```javascript
const express = require('express');
const router = express.Router();
const path = require('path');
const logger = require('../utils/logger');

/**
 * 下载退款 CSV 文件
 */
router.get('/:filename', (req, res) => {
  const filename = req.params.filename;

  // 安全检查：防止路径穿越攻击
  if (filename.includes('..') || filename.includes('/')) {
    return res.status(400).send('非法文件名');
  }

  const filepath = path.join(__dirname, '../../temp', filename);

  logger.info('下载退款文件', { filename });

  res.download(filepath, (err) => {
    if (err) {
      logger.error('文件下载失败', err);
      res.status(404).send('文件不存在');
    }
  });
});

module.exports = router;
```

---

## 智能匹配算法

### 算法流程

```
输入: 知识星球用户列表 + 微信支付订单列表

FOR EACH 用户 IN 用户列表:
    bestMatch = null
    highestScore = 0

    FOR EACH 订单 IN 订单列表:
        score = 0

        # 1. 昵称匹配 (60%)
        nicknameScore = CompareNicknames(用户昵称, 付款人昵称)
        score += nicknameScore * 0.6

        # 2. 时间匹配 (20%)
        timeScore = CompareTime(加入时间, 支付时间)
        score += timeScore * 0.2

        # 3. 金额匹配 (20%)
        amountScore = CompareAmount(押金金额, 支付金额)
        score += amountScore * 0.2

        IF score > highestScore:
            highestScore = score
            bestMatch = 订单

    IF highestScore >= 50:
        匹配成功, 置信度 = highestScore
    ELSE:
        匹配失败

输出: { 匹配列表, 未匹配列表 }
```

### 昵称相似度算法

使用 **Levenshtein 距离算法** 计算两个字符串的相似度：

```javascript
function levenshteinDistance(str1, str2) {
  const matrix = [];

  // 初始化矩阵
  for (let i = 0; i <= str2.length; i++) {
    matrix[i] = [i];
  }
  for (let j = 0; j <= str1.length; j++) {
    matrix[0][j] = j;
  }

  // 动态规划填充矩阵
  for (let i = 1; i <= str2.length; i++) {
    for (let j = 1; j <= str1.length; j++) {
      if (str2.charAt(i - 1) === str1.charAt(j - 1)) {
        matrix[i][j] = matrix[i - 1][j - 1];
      } else {
        matrix[i][j] = Math.min(
          matrix[i - 1][j - 1] + 1,  // 替换
          matrix[i][j - 1] + 1,      // 插入
          matrix[i - 1][j] + 1       // 删除
        );
      }
    }
  }

  return matrix[str2.length][str1.length];
}

// 相似度 = 1 - (距离 / 最大长度)
function similarity(str1, str2) {
  const distance = levenshteinDistance(str1, str2);
  const maxLen = Math.max(str1.length, str2.length);
  return (1 - distance / maxLen) * 100;
}
```

**示例**：

| str1 | str2 | 距离 | 相似度 |
|------|------|------|--------|
| 球球 | 球球 | 0 | 100% |
| 球球的副业探索路 | 球球 | 6 | 33% |
| 小明同学 | 小明 | 2 | 50% |
| Aaron | Aaron.Wang | 5 | 54% |

---

## 错误处理与容错

### 错误分类

| 错误类型 | 处理策略 | 用户提示 |
|---------|---------|---------|
| 数据库错误 | 重试 3 次，失败则降级到只读模式 | "系统繁忙，请稍后重试" |
| 知识星球 API 失败 | 重试 2 次，失败则返回错误 | "知识星球API异常" |
| 微信支付 API 失败 | 降级到人工匹配 | "自动匹配失败，请人工审核" |
| 文件生成失败 | 重试 1 次，记录日志 | "文件生成失败" |
| 参数校验失败 | 立即返回 400 错误 | 具体参数错误信息 |

### 容错机制

```javascript
// 通用重试函数
async function retryAsync(fn, maxRetries = 3, delay = 1000) {
  let lastError;

  for (let i = 0; i < maxRetries; i++) {
    try {
      return await fn();
    } catch (error) {
      lastError = error;
      logger.warn(`重试 ${i + 1}/${maxRetries}`, error);

      if (i < maxRetries - 1) {
        await new Promise(resolve => setTimeout(resolve, delay));
      }
    }
  }

  throw lastError;
}

// 使用示例
const payments = await retryAsync(
  () => wechatPaymentService.getTradeBill(billDate),
  3,
  2000
);
```

---

## 性能优化

### 1. 数据库索引优化

```sql
-- 优化查询性能
CREATE INDEX idx_planet_user ON payment_mapping(planet_user_id, checkin_id);
CREATE INDEX idx_checkin ON payment_mapping(checkin_id);
CREATE INDEX idx_payment_time ON payment_mapping(payment_time);
```

### 2. 批量查询优化

```javascript
// ❌ 不好的做法：N+1 查询
for (const user of users) {
  const payment = db.prepare('SELECT * FROM payment_mapping WHERE planet_user_id = ?')
    .get(user.planet_user_id);
}

// ✅ 好的做法：批量查询
const userIds = users.map(u => u.planet_user_id);
const payments = db.prepare(`
  SELECT * FROM payment_mapping
  WHERE planet_user_id IN (${userIds.map(() => '?').join(',')})
`).all(...userIds);
```

### 3. 并发处理优化

```javascript
const pLimit = require('p-limit');
const limit = pLimit(10);  // 最多 10 个并发

// 并发处理多个用户匹配
const promises = users.map(user =>
  limit(() => smartMatchingService.findBestMatch(user, payments))
);

const results = await Promise.all(promises);
```

### 4. 缓存优化

```javascript
// 缓存训练营信息（避免重复查询）
const checkinCache = new Map();

function getCheckinAmount(checkinId) {
  if (checkinCache.has(checkinId)) {
    return checkinCache.get(checkinId);
  }

  const amount = fetchCheckinAmount(checkinId);  // 从 API 获取
  checkinCache.set(checkinId, amount);
  return amount;
}
```

---

## 安全设计

### 1. 微信支付签名验证

```javascript
function verifySignature(timestamp, nonce, body, signature) {
  const message = `${timestamp}\n${nonce}\n${body}\n`;

  const verify = crypto.createVerify('RSA-SHA256');
  verify.update(message);

  return verify.verify(wechatPublicKey, signature, 'base64');
}
```

### 2. 防止 SQL 注入

```javascript
// ✅ 使用参数化查询
const stmt = db.prepare('SELECT * FROM payment_mapping WHERE planet_user_id = ?');
stmt.get(planetUserId);

// ❌ 永远不要拼接 SQL
// const sql = `SELECT * FROM payment_mapping WHERE planet_user_id = '${planetUserId}'`;
```

### 3. 文件下载路径安全

```javascript
// 防止路径穿越攻击
function sanitizeFilename(filename) {
  if (filename.includes('..') || filename.includes('/')) {
    throw new Error('非法文件名');
  }
  return filename;
}
```

### 4. 敏感数据脱敏

```javascript
// 日志中脱敏处理
function maskSensitiveData(data) {
  return {
    ...data,
    transaction_id: data.transaction_id?.replace(/(.{6})(.*)(.{4})/, '$1****$3'),
    out_trade_no: data.out_trade_no?.replace(/(.{6})(.*)(.{4})/, '$1****$3')
  };
}

logger.info('支付映射保存', maskSensitiveData(data));
```

---

## 测试策略

### 单元测试

```javascript
// tests/unit/smart-matching.test.js
describe('SmartMatchingService', () => {
  test('昵称完全匹配应返回 100 分', () => {
    const score = service.compareNicknames('球球', '球球');
    expect(score).toBe(100);
  });

  test('昵称包含关系应返回 80 分', () => {
    const score = service.compareNicknames('球球的副业探索路', '球球');
    expect(score).toBe(80);
  });

  test('完全不匹配应返回低分', () => {
    const score = service.compareNicknames('张三', '李四');
    expect(score).toBeLessThan(30);
  });
});
```

### 集成测试

```javascript
// tests/integration/refund-flow.test.js
describe('退款流程集成测试', () => {
  test('完整流程：从保存映射到生成退款文件', async () => {
    // 1. 保存支付映射
    await paymentMappingService.saveMapping({
      out_trade_no: 'TEST_ORDER_001',
      planet_user_id: '88455815452182',
      // ...
    });

    // 2. 生成退款文件
    const result = await refundFileService.generateRefundFile(
      '842448118',
      7
    );

    // 3. 验证结果
    expect(result.statistics.auto_matched).toBeGreaterThan(0);
    expect(result.csv_filename).toMatch(/^refund_.*\.csv$/);
  });
});
```

---

## 相关文档

- [README.md](17-v0.2-版本说明.md) - v0.2 版本概述
- [用户旅程图.md](18-v0.2-用户旅程图.md) - 完整的用户流程图
- [数据库设计.md](./数据库设计.md) - SQLite 表结构设计
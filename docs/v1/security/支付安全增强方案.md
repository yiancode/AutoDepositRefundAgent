...
> **文档版本**: v1.1
> **最后更新**: 2025-12-06
> **SSOT引用**: [状态枚举定义.md](../design/状态枚举定义.md) - pay_status 等状态值定义
> **文档目的**：详细说明微信支付的安全加固措施
> **对应决策**：[优化完成总结](../archive/优化完成总结.md) P1-3（支付安全增强）

---

## 📖 术语说明

本文档与接口文档中使用的访问令牌相关术语统一如下：

| 术语 | 说明 | 使用场景 |
|------|------|---------|
| **accessToken** | 临时访问令牌（用户侧名称） | API 接口文档、前端代码、用户说明 |
| **ticket** | 令牌的后端实现机制 | 安全方案设计、Redis 存储结构 |
| **X-Access-Token** | HTTP Header 名称 | 接口请求头 |

**关系说明**：
- `accessToken` 是对外的统一称呼，代表"临时访问令牌"
- `ticket` 是内部实现细节，描述 Token 在 Redis 中的存储方式和验证机制
- 两者本质上指同一个概念，只是从不同视角描述（用户视角 vs 实现视角）

**示例**：
```http
GET /api/h5/orders/{orderNo}/status
Header: X-Access-Token: tk_abc123xyz456

# 后端处理流程：
# 1. 从 Header 提取 accessToken = "tk_abc123xyz456"
# 2. 使用 ticket 机制从 Redis 验证：GET access_token:tk_abc123xyz456
# 3. 验证通过后返回订单状态
```
...
sequenceDiagram
    ...
    H5->>API: POST /api/h5/payments（创建订单）
    API->>Redis: 生成一次性ticket（tk_once_xxx, TTL=5分钟）
    API->>Redis: 关联ticket → orderNo
    API-->>H5: 返回{orderNo, prepay_id, ticket}

    Note over H5,Redis: 支付成功后轮询订单状态

    H5->>API: GET /api/h5/orders/{orderNo}/status?ticket=tk_once_xxx
    API->>Redis: 验证ticket是否存在
    Redis-->>API: 返回orderNo
    API->>API: 验证ticket关联的orderNo === 请求orderNo
    API->>Redis: 删除ticket（一次性使用）
    API-->>H5: 返回订单状态

    H5->>API: GET /api/h5/orders/{orderNo}/status?ticket=tk_once_xxx
    API->>Redis: 查询ticket
    Redis-->>API: ticket不存在
    API-->>H5: 返回401 "票据无效或已使用"
```
...
- [接口文档 - 3.9 支付恢复接口](../api/接口文档.md)
- [技术方案 - 5.2.8 异常处理](../design/技术方案.md)
- [OAuth绑定完整时序图](../diagrams/OAuth绑定完整时序图.md)
...
| 版本 | 日期 | 变更内容 |
|------|------|----------|
| v1.1 | 2025-12-06 | 添加 SSOT 引用；修复 pay_status 字符串值为小写；修复相关文档链接路径 |
